...
class Hero extends InventoryHaver {
  ...
  add_xp({ xp_to_add, use_bonus = true }) {
    ...
    if (xp_to_add + character.xp.current_xp < character.xp.xp_to_next_lvl) {
      // no levelup
      ...
      while (character.xp.total_xp >= character.xp.total_xp_to_next_lvl) {
        ...
        character.xp.total_xp_to_next_lvl = Math.round(
          (character.xp.base_xp_cost *
            (1 - character.xp.xp_scaling ** (level_after_xp + 1))) /
            (1 - character.xp.xp_scaling)
        );
      } //calculates lvl reached after adding xp

      let total_xp_to_previous_lvl = Math.round(
        (character.xp.base_xp_cost *
          (1 - character.xp.xp_scaling ** level_after_xp)) /
          (1 - character.xp.xp_scaling)
      );
      ...
      character.xp.xp_to_next_lvl =
        character.xp.total_xp_to_next_lvl - total_xp_to_previous_lvl;
      character.xp.current_level = level_after_xp;
      character.xp.current_xp =
        character.xp.total_xp - total_xp_to_previous_lvl;
  ...
  get_level_bonus(level) {
    ...
    for (let i = character.xp.current_level + 1; i <= level; i++) {
      ...
      gained_stamina += 5 * Math.ceil(i / 10);
      total_skill_xp_multiplier =
        total_skill_xp_multiplier * gained_skill_xp_multiplier;
    }

    character.stats.flat.level.max_health =
      (character.stats.flat.level.max_health || 0) + gained_hp;
    character.stats.flat.level.health = character.stats.flat.level.max_health;
    character.stats.flat.level.max_stamina =
      (character.stats.flat.level.max_stamina || 0) + gained_stamina;
    character.stats.flat.level.stamina = character.stats.flat.level.max_stamina;
    character.stats.flat.level.strength =
      (character.stats.flat.level.strength || 0) + gained_str;
    character.stats.flat.level.intuition =
      (character.stats.flat.level.intuition || 0) + gained_int;
    character.stats.flat.level.agility =
      (character.stats.flat.level.agility || 0) + gained_agi;
    character.stats.flat.level.dexterity =
      (character.stats.flat.level.dexterity || 0) + gained_dex;
    ...
    gains += `<br>Skill xp gains increased by ${Math.round(
      (gained_skill_xp_multiplier - 1) * 100
    )}%`;
 ...
 */
character.stats.add_skill_milestone_bonus = function ({
  stats = {},
  xp_multipliers = {},
}) {
  Object.keys(stats).forEach((stat) => {
    if (stats[stat].flat) {
      character.stats.flat.skill_milestones[stat] =
        (character.stats.flat.skill_milestones[stat] || 0) + stats[stat].flat;
    }
    if (stats[stat].multiplier) {
      character.stats.multiplier.skill_milestones[stat] =
        (character.stats.multiplier.skill_milestones[stat] || 1) *
        stats[stat].multiplier;
    }
  });

  if (xp_multipliers?.hero) {
    character.xp_bonuses.multiplier.skills.hero =
      (character.xp_bonuses.multiplier.skills.hero || 1) * xp_multipliers.hero;
  }
  if (xp_multipliers?.all) {
    character.xp_bonuses.multiplier.skills.all =
      (character.xp_bonuses.multiplier.skills.all || 1) * xp_multipliers.all;
  }
  if (xp_multipliers?.all_skill) {
    character.xp_bonuses.multiplier.skills.all_skill =
      (character.xp_bonuses.multiplier.skills.all_skill || 1) *
      xp_multipliers.all_skill;
  }

  Object.keys(skills).forEach((skill) => {
    if (xp_multipliers[skill]) {
      character.xp_bonuses.multiplier.skills[skill] =
        (character.xp_bonuses.multiplier.skills[skill] || 1) *
        xp_multipliers[skill];
    }
  });
  Object.keys(skill_categories).forEach((category) => {
    const cat = "category_" + category;
    if (xp_multipliers[cat]) {
      character.xp_bonuses.multiplier.skills[cat] =
        (character.xp_bonuses.multiplier.skills[cat] || 1) *
        xp_multipliers[cat];
    }
  });
};
 ...
 * @param {{flats, multipliers}} bonuses
 */
character.stats.add_book_bonus = function ({
  multipliers = {},
  xp_multipliers = {},
}) {
  Object.keys(character.base_stats).forEach((stat) => {
    if (multipliers[stat]) {
      character.stats.multiplier.books[stat] =
        (character.stats.multiplier.books[stat] || 1) * multipliers[stat];
    }
  });
...
character.stats.add_active_effect_bonus = function () {
  ...
  Object.keys(active_effects).forEach((effect_key) => {
    ...
    for (const [key, value] of Object.entries(effects.stats)) {
      if (value.flat) {
        character.stats.flat.active_effect[key] =
          (character.stats.flat.active_effect[key] || 0) +
          value.flat * multiplier;
      }

      if (value.multiplier) {
        character.stats.multiplier.active_effect[key] =
          (character.stats.multiplier.active_effect[key] || 1) *
          value.multiplier *
          multiplier;
      }
    }
    for (const [key, value] of Object.entries(effects.bonus_skill_levels)) {
      character.bonus_skill_levels.flat.active_effects[key] =
        (character.bonus_skill_levels.flat.active_effects[key] || 0) + value;
    }
  });
};
...
character.stats.add_all_equipment_bonus = function () {
  ...
  Object.keys(character.equipment).forEach((slot) => {
    ...
    if (character.equipment[slot].getDefense) {
      character.stats.flat.equipment.defense =
        (character.stats.flat.equipment.defense || 0) +
        character.equipment[slot].getDefense();
    }
    ...
    Object.keys(stats).forEach((stat) => {
      if (stats[stat].flat) {
        character.stats.flat.equipment[stat] =
          (character.stats.flat.equipment[stat] || 0) + stats[stat].flat;
      }

      if (stats[stat].multiplier) {
        character.stats.multiplier.equipment[stat] =
    ...
    Object.keys(bonuses).forEach((bonus) => {
      if (bonuses[bonus]) {
        character.bonus_skill_levels.flat.equipment[bonus] =
          (character.bonus_skill_levels.flat.equipment[bonus] || 0) +
          bonuses[bonus];
      }
    });
  });
...
character.stats.add_weapon_type_bonuses = function () {
  if (character.equipment.weapon == null) {
    character.stats.multiplier.skills.attack_power =
      get_total_skill_coefficient({ skill_id: "Unarmed" });
    character.stats.multiplier.skills.attack_speed =
      get_total_skill_coefficient({
        scaling_type: "multiplicative",
        skill_id: "Unarmed",
      }) ** 0.3333;
    character.stats.multiplier.skills.attack_points =
      get_total_skill_coefficient({
        scaling_type: "multiplicative",
        skill_id: "Unarmed",
      }) ** 0.3333;
    character.stats.multiplier.skills.evasion_points =
      get_total_skill_coefficient({
        scaling_type: "multiplicative",
        skill_id: "Unarmed",
      }) ** 0.3333;
    ...
    character.stats.multiplier.skills.attack_power =
      get_total_skill_coefficient({
        skill_id: weapon_type_to_skill[character.equipment.weapon.weapon_type],
      });
    character.stats.multiplier.skills.attack_points =
      get_total_skill_coefficient({
        skill_id: weapon_type_to_skill[character.equipment.weapon.weapon_type],
...
character.stats.add_all_skill_level_bonus = function () {
  ...
  character.stats.multiplier.skills.stamina_efficiency =
    get_total_skill_coefficient({
      scaling_type: "multiplicative",
      skill_id: "Running",
    });

  character.stats.multiplier.skills.strength =
    get_total_skill_coefficient({
      scaling_type: "multiplicative",
  ...
  character.stats.multiplier.skills.block_strength =
    1 + 5 * get_total_level_bonus("Shield blocking");

  character.stats.multiplier.skills.agility =
    get_total_skill_coefficient({
      scaling_type: "multiplicative",
  ...
  character.stats.multiplier.skills.dexterity = get_total_skill_coefficient({
    scaling_type: "multiplicative",
    skill_id: "Climbing",
  });

  character.stats.multiplier.skills.max_stamina = get_total_skill_coefficient({
    scaling_type: "multiplicative",
    skill_id: "Breathing",
  });
...
character.stats.add_location_penalties = function () {
  ...
  if (current_location) {
    ...
    if (
      current_location.light_level === "dark" ||
      (current_location.light_level === "normal" && is_night(current_game_time))
    ) {
      light_modifier =
        0.5 +
        (0.5 * get_total_skill_level("Night vision")) /
  ...
  Object.keys(effects.multipliers || {}).forEach((effect) => {
    character.stats.multiplier.environment[effect] =
      effects.multipliers[effect];
  });
...
character.update_stats = function () {
  ...
  const missing_stamina =
    Math.max(
      character.stats.full["max_stamina"] - character.stats.full["stamina"],
      0
    ) || 0;
  const missing_mana =
    Math.max(
      character.stats.full["max_mana"] - character.stats.full["mana"],
      0
    ) || 0;
  ...
  Object.keys(character.stats.full).forEach((stat) => {
    ...
    if (stat === "block_chance") {
      stat_sum =
        base_block_chance +
        Math.round(get_total_level_bonus("Shield blocking") * 10000) / 10000;
    } else if (stat === "attack_points") {
      stat_sum =
        Math.sqrt(character.stats.full.intuition) *
        character.stats.full.dexterity *
        get_total_skill_coefficient({
          scaling_type: "multiplicative",
          skill_id: "Combat",
        });
    ...
    character.stats.full[stat] =
      (character.base_stats[stat] + stat_sum) * stat_mult;
    ...
    if (stat === "health") {
      ...
      character.stats.full.stamina = Math.max(
        0,
        character.stats.full["max_stamina"] - missing_stamina
      );
    } else if (stat === "mana") {
      character.stats.full.mana = Math.max(
        0,
        character.stats.full["max_mana"] - missing_mana
      );
    }
  });

  if (character.equipment.weapon != null) {
    character.stats.full.attack_power =
      (character.stats.full.strength / 10) *
      character.equipment.weapon.getAttack() *
      character.stats.total_multiplier.attack_power;
  } else {
    character.stats.full.attack_power =
      (character.stats.full.strength / 10) *
      character.stats.total_multiplier.attack_power;
  ...
  character.stats.total_flat.attack_power =
    character.stats.full.attack_power /
    character.stats.total_multiplier.attack_power;
  Object.keys(character.xp_bonuses.total_multiplier).forEach((bonus_target) => {
    ...
    if (bonus != 1) {
      if (bonus_target !== "hero") {
        if (
          bonus_target === "all" ||
          bonus_target === "all_skill" ||
          bonus_target.includes("category_")
        ) {
          update_all_displayed_skills_xp_gain();
        } else {
...
character.get_attack_speed = function () {
  let spd =
    character.stats.full.attack_speed * character.get_stamina_multiplier();
  return spd;
};
...
character.wears_armor = function () {
  return (
    ...
    (character.equipment.torso &&
      character.equipment.torso.getDefense() !== 0) ||
...
character.take_damage = function ({
  ...
  damage_values = damage_values.map((val) => {
    if (val < 1) {
      ...
      return (
        Math.ceil(
          10 * Math.max(val - character.stats.full.defense, val * 0.05, 1)
        ) / 10
      );
    }
  });
...
function equip_item_from_inventory(item_key) {
  if (item_key in character.inventory) {
    //check if its in inventory, just in case
...
function get_skill_xp_gain(skill_name) {
  ...
  return (
    (character.xp_bonuses.total_multiplier.all_skill || 1) *
    (character.xp_bonuses.total_multiplier.all || 1) *
...
function get_total_skill_level(skill_id) {
  return (
    skills[skill_id].current_level +
    (character.bonus_skill_levels.full[skill_id] || 0)
  );
}
...
function get_total_skill_coefficient({ scaling_type, skill_id }) {
  return skills[skill_id].get_coefficient({
    scaling_type,
    skill_level: get_total_skill_level(skill_id),
  });
}
...
function get_effect_with_bonuses(active_effect) {
  ...
  if (active_effect.tags.medicine) {
    multiplier *= get_total_skill_coefficient({
      scaling_type: "multiplicative",
      skill_id: "Medicine",
    });
  }
  let boosted = {
    stats: {},
    bonus_skill_levels: { ...active_effect.effects.bonus_skill_levels },
  };
