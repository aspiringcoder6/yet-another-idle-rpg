...
import {
  skills,
  get_unlocked_skill_rewards,
  get_next_skill_milestone,
} from "./skills.js";
import {
  character,
  get_skill_xp_gain,
  get_hero_xp_gain,
  get_skills_overall_xp_gain,
  get_total_skill_coefficient,
  get_total_skill_level,
  get_effect_with_bonuses,
} from "./character.js";
import {
  current_enemies,
  options,
...
const current_stamina_value_div = document.getElementById(
  "character_stamina_value"
);
const current_stamina_bar = document.getElementById(
...
const stats_divs = {
  strength: document.getElementById("strength_slot"),
  agility: document.getElementById("agility_slot"),
  dexterity: document.getElementById("dexterity_slot"),
  intuition: document.getElementById("intuition_slot"),
  ...
  defense: document.getElementById("defense_slot"),
  crit_rate: document.getElementById("crit_rate_slot"),
  crit_multiplier: document.getElementById("crit_multiplier_slot"),
};

const other_combat_divs = {
  attack_points: document.getElementById("hit_chance_slot"),
  defensive_action: document.getElementById("defensive_action_slot"),
...
//equipment slots
const equipment_slots_divs = {
  head: document.getElementById("head_slot"),
  torso: document.getElementById("torso_slot"),
  arms: document.getElementById("arms_slot"),
  ring: document.getElementById("ring_slot"),
  weapon: document.getElementById("weapon_slot"),
  "off-hand": document.getElementById("off-hand_slot"),
  legs: document.getElementById("legs_slot"),
  feet: document.getElementById("feet_slot"),
  amulet: document.getElementById("amulet_slot"),
  artifact: document.getElementById("artifact_slot"),
...
const crafting_pages = {
  crafting: {
    items: document.querySelector(
      `[data-crafting_category="crafting"] [data-crafting_subcategory="items"]`
    ),
    components: document.querySelector(
      `[data-crafting_category="crafting"] [data-crafting_subcategory="components"]`
    ),
    equipment: document.querySelector(
      `[data-crafting_category="crafting"] [data-crafting_subcategory="equipment"]`
    ),
  },
  cooking: {
    items: document.querySelector(
      `[data-crafting_category="cooking"] [data-crafting_subcategory="items"]`
    ),
  },
  smelting: {
    items: document.querySelector(
      `[data-crafting_category="smelting"] [data-crafting_subcategory="items"]`
    ),
  },
  forging: {
    items: document.querySelector(
      `[data-crafting_category="forging"] [data-crafting_subcategory="items"]`
    ),
    components: document.querySelector(
      `[data-crafting_category="forging"] [data-crafting_subcategory="components"]`
    ),
  },
  alchemy: {
    items: document.querySelector(
      `[data-crafting_category="alchemy"] [data-crafting_subcategory="items"]`
    ),
  },
};

const backup_load_button = document.getElementById("backup_load_button");
const other_save_load_button = document.getElementById(
  "import_other_save_button"
);
...
function create_item_tooltip_content({ item, options = {} }) {
  ...
  if (item.item_type === "EQUIPPABLE") {
    ...
    if (!options.skip_quality && options?.quality?.length == 2) {
      item_tooltip += `<br><br><b>Quality: <span style="color: ${
        rarity_colors[item.getRarity(options.quality[0])]
      }"> ${options.quality[0]}% </span> - <span style="color: ${
        rarity_colors[item.getRarity(options.quality[1])]
      }"> ${options.quality[1]}% </span>`;
      item_tooltip += `<br>[<span style="color: ${
        rarity_colors[item.getRarity(options.quality[0])]
      }">${item.getRarity(options.quality[0])}</span>-<span style="color: ${
        rarity_colors[item.getRarity(options.quality[1])]
      }">${item.getRarity(options.quality[1])}</span>] </b>`;
    } else {
      item_tooltip += `<br><br><b style="color: ${
        rarity_colors[item.getRarity(quality)]
      }">Quality: ${quality}% [${item.getRarity(quality)}]</b>`;
    }
    ...
    if (item.components) {
      ...
      if (item.components) {
        component_description += `[${
          item_templates[item.components[components[0]]].name
        }]`;
        if (!item.components[components[1]]) {
          component_description += `<br>+<br>no [${components[1]}]`;
        } else {
          component_description += `<br>+<br>[${
            item_templates[item.components[components[1]]].name
          }]`;
        }
      }
    ...
    if (!options.skip_quality && options?.quality?.length == 2) {
      if (item.getAttack) {
        item_tooltip += `<br><br>Attack: ${
          Math.round(10 * item.getAttack(options.quality[0]), true) / 10
        } - ${Math.round(10 * item.getAttack(options.quality[1], true)) / 10}`;
      } else if (item.getDefense) {
        item_tooltip += `<br><br>Defense: ${
          Math.round(10 * item.getDefense(options.quality[0])) / 10
        } - ${Math.round(10 * item.getDefense(options.quality[1])) / 10}`;
      } else if (item.offhand_type === "shield") {
        item_tooltip += `<br><br>Can block up to: ${
          Math.round(
            10 *
              item.getShieldStrength(options.quality[0]) *
              character.stats.total_multiplier.block_strength
          ) / 10
        } - ${
          Math.round(
            10 *
              item.getShieldStrength(options.quality[1]) *
              character.stats.total_multiplier.block_strength
          ) / 10
        } damage [base: ${item.getShieldStrength(
          options.quality[0]
        )}-${item.getShieldStrength(options.quality[1])}]`;
      }
      ...
      Object.keys(equip_stats_0).forEach((effect_key) => {
        if (equip_stats_0[effect_key].flat != null) {
          item_tooltip += `<br>${capitalize_first_letter(effect_key).replace(
            "_",
            " "
          )}: +${equip_stats_0[effect_key].flat} - ${
            equip_stats_1[effect_key].flat
          }`;
        }
        if (equip_stats_0[effect_key].multiplier != null) {
          item_tooltip += `<br>${capitalize_first_letter(effect_key).replace(
            "_",
            " "
          )}: x${equip_stats_0[effect_key].multiplier} - ${
            equip_stats_1[effect_key].multiplier
          }`;
        }
      });
    } else {
      if (item.getAttack) {
        item_tooltip += `<br><br>Attack: ${
          Math.round(10 * item.getAttack()) / 10
        }`;
      } else if (item.getDefense) {
        item_tooltip += `<br><br>Defense: ${
          Math.round(10 * item.getDefense()) / 10
        }`;
      } else if (item.offhand_type === "shield") {
        item_tooltip += `<br><br>Can block up to: ${
          Math.round(
            10 *
              item.getShieldStrength() *
              character.stats.total_multiplier.block_strength
          ) / 10
        } damage [base: ${item.getShieldStrength()}]`;
      }
      ...
      Object.keys(equip_stats).forEach(function (effect_key) {
        if (equip_stats[effect_key].flat != null) {
          item_tooltip += `<br>${capitalize_first_letter(effect_key).replace(
            "_",
            " "
          )}: +${equip_stats[effect_key].flat}`;
        }
        if (equip_stats[effect_key].multiplier != null) {
          item_tooltip += `<br>${capitalize_first_letter(effect_key).replace(
            "_",
            " "
          )}: x${equip_stats[effect_key].multiplier}`;
        }
      });
    }
    ...
    Object.keys(equip_bonus_skill_levels).forEach((skill_key) => {
      if (skill_key.includes("category_")) {
        ...
        item_tooltip += `<br>${skills[skill_key].name()} level: +${
          equip_bonus_skill_levels[skill_key]
        }`;
      }
    });
    ...
    for (let i = 0; i < item.effects.length; i++) {
      item_tooltip += create_effect_tooltip({
        effect_name: item.effects[i].effect,
        duration: item.effects[i].duration,
        add_bonus: true,
      }).outerHTML;
    }
    ...
    if (!book_stats[item.name].is_finished) {
      ...
      if (Object.keys(book_stats[item.name].bonuses).length > 0) {
        item_tooltip += `<br>- ${format_book_bonuses(
          book_stats[item.name].bonuses
        )}`;
      }
    ...
    if (
      Object.keys(item.component_stats).length > 0 ||
      item?.attack_value !== 0 ||
      item?.attack_multiplier !== 1 ||
    ...
    Object.keys(item.component_stats).forEach(function (effect_key) {
      if (item.component_stats[effect_key].flat != null) {
        item_tooltip += `<br>${capitalize_first_letter(effect_key).replace(
          "_",
          " "
        )}: +${item.component_stats[effect_key].flat}`;
      }
      if (item.component_stats[effect_key].multiplier != null) {
        item_tooltip += `<br>${capitalize_first_letter(effect_key).replace(
          "_",
          " "
        )}: x${item.component_stats[effect_key].multiplier}`;
      }
    });
    item_tooltip += "<br>";
  } else {
  ...
  if (!options.skip_quality && options?.quality?.length == 2) {
    item_tooltip += `<br>Value: ${format_money(
      round_item_price(item.getValue(options.quality[0]))
    )} - ${format_money(round_item_price(item.getValue(options.quality[1])))}`;
  } else {
    item_tooltip += `<br>Value: ${format_money(
      round_item_price(
        item.getValue(quality) *
          (options && options.trader
            ? traders[current_trader].getProfitMargin()
            : 1) || 1
      )
    )}`;
  }
  if (item.saturates_market) {
    item_tooltip += ` [originally ${format_money(
      round_item_price(
        item.getBaseValue(quality) *
          (options && options.trader
            ? traders[current_trader].getProfitMargin()
            : 1) || 1
      )
...
function create_effect_tooltip({ effect_name, duration, add_bonus = false }) {
  ...
  for (const [key, stat_value] of Object.entries(effects.stats)) {
    tooltip.innerHTML += `<br>${capitalize_first_letter(
      key.replaceAll("_", " ").replace("flat", "").replace("percent", "")
    )} `;
    //for regeneration bonuses, it is assumed they are only flat and not multiplicative
    if (
      key === "health_regeneration_flat" ||
      key === "stamina_regeneration_flat" ||
      key === "mana_regeneration_flat" ||
      key === "health_loss_flat"
    ) {
      const sign = stat_value.flat > 0 ? "+" : "";
      tooltip.innerHTML += `: ${sign}${
        Math.round(100 * stat_value.flat) / 100
      }`;
    } else if (
      key === "health_regeneration_percent" ||
      key === "stamina_regeneration_percent" ||
      key === "mana_regeneration_percent" ||
      key === "health_loss_percent"
    ) {
...
function format_book_bonuses(bonuses) {
  ...
  if (bonuses.multipliers) {
    ...
    if (formatted) {
      formatted += `, x${bonuses.multipliers[multipliers[0]]} ${
        stat_names[multipliers[0]]
      }`;
    } else {
      formatted = `x${bonuses.multipliers[multipliers[0]]} ${
        stat_names[multipliers[0]]
    ...
    for (let i = 1; i < multipliers.length; i++) {
      formatted += `, x${bonuses.multipliers[multipliers[i]]} ${
        stat_names[multipliers[i]]
      }`;
    }
  }
  if (bonuses.xp_multipliers) {
    ...
    let name;
    if (
      xp_multipliers[0] !== "all" &&
      xp_multipliers[0] !== "hero" &&
      xp_multipliers[0] !== "all_skill"
    ) {
    ...
    if (formatted) {
      formatted += `, x${
        bonuses.xp_multipliers[xp_multipliers[0]]
      } ${name} xp gain`;
    } else {
      formatted = `x${
        bonuses.xp_multipliers[xp_multipliers[0]]
    ...
    for (let i = 1; i < xp_multipliers.length; i++) {
      let name;
      if (
        xp_multipliers[i] !== "all" &&
        xp_multipliers[i] !== "hero" &&
        xp_multipliers[i] !== "all_skill"
      ) {
      ...
      formatted += `, x${
        bonuses.xp_multipliers[xp_multipliers[i]]
      } ${name} xp gain`;
    }
  }
...
function start_activity_animation(settings) {
  ...
  activity_anim = setInterval(() => {
    //sets a tiny little "animation" for activity text
    ...
    } else if (action_status_div.innerHTML.endsWith(".")) end = ".";

    if (settings?.book_title) {
      action_status_div.innerHTML =
        action_status_div.innerHTML.split(",")[0] +
        `, ${format_reading_time(
          item_templates[settings.book_title].getRemainingTime()
        )} left`;
    ...
    if (end.length < 3) {
      ...
      action_status_div.innerHTML = action_status_div.innerHTML.substring(
        0,
        action_status_div.innerHTML.length - 3
      );
    }
  }, 600);
}
...
function update_displayed_trader(is_newly_open) {
  ...
  document.getElementById("trader_cost_mult_value").textContent = `${Math.round(
    100 * traders[current_trader].getProfitMargin()
  )}%`;
...
function update_displayed_money() {
  document.getElementById(
    "money_div"
  ).innerHTML = `Your purse contains: ${format_money(character.money)}`;
}
...
function sort_displayed_inventory({
  sort_by = "name",
  target = "character",
  ...
  [...target.children]
    .sort((a, b) => {
      ...
      if (
        a.classList.contains("equipped_item_control") &&
        !b.classList.contains("equipped_item_control")
      ) {
        return -1;
      } else if (
        !a.classList.contains("equipped_item_control") &&
      ...
      if (
        a.classList.contains("item_to_trade") &&
        !b.classList.contains("item_to_trade")
      ) {
        return 1;
      } else if (
        !a.classList.contains("item_to_trade") &&
      ...
      if (
        a.classList.contains("character_item_equippable") &&
        !b.classList.contains("character_item_equippable")
      ) {
        return 1;
      } else if (
        !a.classList.contains("character_item_equippable") &&
      ...
      if (
        a.classList.contains("trader_item_equippable") &&
        !b.classList.contains("trader_item_equippable")
      ) {
        return 1;
      } else if (
        !a.classList.contains("trader_item_equippable") &&
      ...
      if (
        a.classList.contains("storage_item_equippable") &&
        !b.classList.contains("storage_item_equippable")
      ) {
        return 1;
      } else if (
        !a.classList.contains("storage_item_equippable") &&
      ...
      if (
        a.children[0].children[0].children[0].innerText === "[Comp]" &&
        b.children[0].children[0].children[0].innerText !== "[Comp]"
      ) {
        return 1;
      } else if (
        a.children[0].children[0].children[0].innerText !== "[Comp]" &&
      ...
      if (
        a.children[0].children[0].children[0].innerText === "[Book]" &&
        b.children[0].children[0].children[0].innerText !== "[Book]"
      ) {
        return 1;
      } else if (
        a.children[0].children[0].children[0].innerText !== "[Book]" &&
      ...
      if (
        a.getElementsByClassName("item_slot") &&
        !b.getElementsByClassName("item_slot")
      ) {
        return 1;
      } else if (
        !a.getElementsByClassName("item_slot") &&
      ...
      if (sort_by === "name") {
        const name_a = a.children[0].children[0].children[1].innerText
          .toLowerCase()
          .replaceAll('"', "");
        const name_b = b.children[0].children[0].children[1].innerText
          .toLowerCase()
          .replaceAll('"', "");
        if (name_a > name_b) {
          return plus;
        ...
        if (value_a > value_b) {
          ...
          if (
            value_a === value_b &&
            "item_quality" in a.dataset &&
            "item_quality" in b.dataset
          ) {
            if (
              Number.parseInt(a.dataset.item_quality) >
              Number.parseInt(b.dataset.item_quality)
            ) {
              return plus;
            } else {
              return minus;
...
function update_displayed_trader_inventory({
  ...
  if (item_key) {
    ...
    trader_item_divs[item_key] = create_inventory_item_div({
      key: item_key,
      item_count,
      target: "trader",
    });
    ...
    Object.keys(trader.inventory).forEach((inventory_key) => {
      ...
      if (!trader_item_divs[inventory_key]) {
        trader_item_divs[inventory_key] = create_inventory_item_div({
          key: inventory_key,
          item_count,
          target: "trader",
        });
        ...
        let div_count = Number.parseInt(
          trader_item_divs[inventory_key]
            .getElementsByClassName("item_count")[0]
            .innerText.replace("x", "")
        );
        ...
        if (div_count != item_count) {
          if (item_count > 1) {
            trader_item_divs[inventory_key].getElementsByClassName(
              "item_count"
            )[0].innerText = ` x${item_count}`;
          } else {
            trader_item_divs[inventory_key].getElementsByClassName(
              "item_count"
            )[0].innerText = ``;
        ...
        if (is_newly_open) {
          ...
          price_span.innerHTML = `${format_money(
            round_item_price(
              trader.inventory[inventory_key].item.getValue() *
                (traders[current_trader].getProfitMargin() || 1)
            ),
            true
          )}`;
        }
    ...
    for (let i = 0; i < to_sell.items.length; i++) {
      ...
      if (!item_selling_divs[key]) {
        item_selling_divs[key] = create_inventory_item_div({
          target: "trader",
          trade_index: i,
        });
        ...
        if (div_count !== item_count) {
          if (item_count > 1) {
            item_selling_divs[key].getElementsByClassName(
              "item_count"
            )[0].innerText = ` x${item_count}`;
          } else {
            item_selling_divs[key].getElementsByClassName(
...
function update_displayed_character_inventory({
  ...
  if (!item_key) {
    Object.keys(item_divs).forEach((div_key) => {
      if (item_divs[div_key].classList.contains("equipped_item_control")) {
        ...
        if (
          !character.equipment[div_key] ||
          item_key !== character.equipment[div_key].getInventoryKey()
        ) {
  ...
  if (item_key) {
    ...
    item_divs[item_key] = create_inventory_item_div({
      key: item_key,
      item_count,
      target: "character",
    });
    ...
    item_divs[equip_slot] = create_inventory_item_div({
      key: equip_slot,
      target: "character",
      is_equipped: true,
    });
    ...
    Object.keys(character.inventory).forEach((inventory_key) => {
      ...
      if (!item_divs[inventory_key]) {
        item_divs[inventory_key] = create_inventory_item_div({
          key: inventory_key,
          item_count,
          target: "character",
        });
        ...
        if (div_count != item_count) {
          if (item_count > 1) {
            item_divs[inventory_key].getElementsByClassName(
              "item_count"
            )[0].innerText = ` x${item_count}`;
          } else {
            item_divs[inventory_key].getElementsByClassName(
    ...
    Object.keys(character.equipment).forEach((eq_slot) => {
      if (!item_divs[eq_slot]) {
        if (character.equipment[eq_slot]) {
          ...
          item_divs[eq_slot] = create_inventory_item_div({
            key: eq_slot,
            target: "character",
            is_equipped: true,
          });
    ...
    for (let i = 0; i < to_buy.items.length; i++) {
      ...
      if (!item_buying_divs[key]) {
        item_buying_divs[key] = create_inventory_item_div({
          target: "character",
          trade_index: i,
        });
        ...
        if (div_count !== item_count) {
          if (item_count > 1) {
            item_buying_divs[key].getElementsByClassName(
              "item_count"
            )[0].innerText = ` x${item_count}`;
          } else {
            item_buying_divs[key].getElementsByClassName(
  ...
  if (was_anything_new_added) {
    sort_displayed_inventory({
      target: "character",
      sort_by: character_sorting,
      direction: sorting_direction,
    });
  }
}

function update_displayed_storage_inventory({
  item_key,
  storage_sorting = "name",
  sorting_direction = "asc",
  was_anything_new_added = false,
} = {}) {
  //removal of unneeded divs
  ...
  if (item_key) {
    ...
    storage_item_divs[item_key] = create_inventory_item_div({
      key: item_key,
      item_count,
      target: "storage",
    });
    ...
    Object.keys(player_storage.inventory).forEach((inventory_key) => {
      ...
      if (!storage_item_divs[inventory_key]) {
        storage_item_divs[inventory_key] = create_inventory_item_div({
          key: inventory_key,
          item_count,
          target: "storage",
        });
        ...
        let div_count = Number.parseInt(
          storage_item_divs[inventory_key]
            .getElementsByClassName("item_count")[0]
            .innerText.replace("x", "")
        );
        if (Number.isNaN(div_count)) {
        ...
        if (div_count != item_count) {
          if (item_count > 1) {
            storage_item_divs[inventory_key].getElementsByClassName(
              "item_count"
            )[0].innerText = ` x${item_count}`;
          } else {
            storage_item_divs[inventory_key].getElementsByClassName(
 ...
 * @returns
 */
function create_inventory_item_div({
  key,
  item_count,
  target,
  is_equipped,
  ...
  if (target === "trader") {
    options.trader = true;
    price_multiplier =
      traders[current_trader].getProfitMargin() || price_multiplier;
  } else if (target === "storage") {
  ...
  if (is_equipped) {
    ...
    if (target === "character") {
      if (typeof trade_index === "undefined") {
        ...
        target_item =
          traders[current_trader].inventory[to_buy.items[trade_index].item_key]
            .item;
      ...
      if (typeof trade_index === "undefined") {
        ...
        target_item =
          character.inventory[to_sell.items[trade_index].item_key].item;
  ...
  if (target_item.tags?.equippable) {
    if (target_item.tags.tool) {
      ...
      item_name_div.innerHTML = `<span class = "item_slot" >[${
        target_item.equip_slot
      }]</span> <span>${target_item.getName()}</span>`;
    }
    item_name_div.classList.add(`${item_class}_name`);
    item_div.appendChild(item_name_div);

    item_control_div.classList.add(
      `${item_class}_control`,
      `${target_class_name}_control`,
      `${target_class_name}_equippable`
    );
    item_control_div.appendChild(item_div);

    if (typeof trade_index !== "undefined") {
      item_div.classList.add(
        `${item_class}`,
        `${target_class_name}`,
  ...
  item_div.classList.add(
    `${item_class}`,
    `${target_class_name}`,
    `item_${target_item.item_type.toLowerCase()}`
  );
  ...
  item_control_div.classList.add(
    `${item_class}_control`,
    `${target_class_name}_control`,
    `${target_class_name}_${target_item.item_type.toLowerCase()}`
  );
  item_control_div.setAttribute(
    `data-${target_class_name}`,
    `${target_item.getInventoryKey()}`
  );
  ...
  item_value_span.innerHTML = `${format_money(
    round_item_price(target_item.getValue() * price_multiplier),
    true
  )}`;
...
function update_displayed_book(book_id) {
  ...
  item_divs[book_key]
    .getElementsByClassName("item_book")[0]
    .appendChild(create_item_tooltip(book));
}
  ...
  for (let i = 0; i < 8; i++) {
    ...
    if (i < current_enemies.length) {
      ...
      if (current_enemies[i].stats.attack_speed > 20) {
        ...
        disp_speed =
          Math.round(current_enemies[i].stats.attack_speed * 10) / 10;
      } else {
        disp_speed =
          Math.round(current_enemies[i].stats.attack_speed * 100) / 100;
      }

      let hero_hit_chance_modifier =
        current_enemies.filter((enemy) => enemy.is_alive).length ** (-1 / 4); // down to ~ 60% if there's full 8 enemies
      if (current_enemies[i].size === "small") {
        hero_hit_chance_modifier *= get_total_skill_coefficient({
          scaling_type: "multiplicative",
          skill_id: "Pest killer",
        });
      }

      let hero_evasion_chance_modifier =
        current_enemies.filter((enemy) => enemy.is_alive).length ** (-1 / 3); //down to .5 if there's full 8 enemies (multiple attackers make it harder to evade attacks)
      if (current_enemies[i].size === "large") {
        hero_evasion_chance_modifier *= get_total_skill_coefficient({
          scaling_type: "multiplicative",
          skill_id: "Giant slayer",
        });
      }

      const evasion_chance =
        1 -
        get_hit_chance(
          character.stats.full.attack_points * hero_hit_chance_modifier,
          current_enemies[i].stats.agility *
            Math.sqrt(current_enemies[i].stats.intuition ?? 1)
        );
      let hit_chance = get_hit_chance(
        current_enemies[i].stats.dexterity *
          Math.sqrt(current_enemies[i].stats.intuition ?? 1),
        character.stats.full.evasion_points * hero_evasion_chance_modifier
      );

      if (character.equipment["off-hand"]?.offhand_type === "shield") {
        //has shield
        hit_chance = 1;
      }

      //enemies_div.children[i].children[0].children[1].innerHTML = `AP : ${Math.round(ap)} | EP : ${Math.round(ep)}`;
      enemies_div.children[
        i
      ].children[0].children[1].children[0].innerHTML = `Atk pwr: ${current_enemies[i].stats.attack}`;
      if (current_enemies[i].stats.attack_count > 1) {
        enemies_div.children[
          i
        ].children[0].children[1].children[0].innerHTML += `x${current_enemies[i].stats.attack_count}`;
      }
      enemies_div.children[
        i
      ].children[0].children[1].children[1].innerHTML = `Atk spd: ${disp_speed}`;
      enemies_div.children[
        i
      ].children[0].children[1].children[2].innerHTML = `Hit: ${Math.min(
        100,
        Math.max(0, Math.round(100 * hit_chance))
      )}%`; //100% if shield!
      enemies_div.children[
        i
      ].children[0].children[1].children[3].innerHTML = `Ddg: ${Math.min(
        100,
        Math.max(0, Math.round(100 * evasion_chance))
      )}%`;
      enemies_div.children[
        i
      ].children[0].children[1].children[4].innerHTML = `Def: ${current_enemies[i].stats.defense}`;
    } else {
...
function update_displayed_health_of_enemies() {
  for (let i = 0; i < current_enemies.length; i++) {
    ...
    enemies_div.children[
      i
    ].children[0].children[2].children[0].children[0].style.width =
      Math.max(
        0,
        (100 * current_enemies[i].stats.health) /
          current_enemies[i].stats.max_health
      ) + "%";

    enemies_div.children[
      i
    ].children[0].children[2].children[1].innerText = `${Math.ceil(
      current_enemies[i].stats.health
    )}/${Math.ceil(current_enemies[i].stats.max_health)} hp`;
  }
}
...
function update_displayed_normal_location(location) {
  ...
  document.documentElement.style.setProperty(
    "--location_desc_tooltip_visibility",
    "hidden"
  );
  ...
  document.documentElement.style.setProperty(
    ...
    getComputedStyle(document.body).getPropertyValue(
      "--actions_div_height_default"
    )
  );
  document.documentElement.style.setProperty(
    "--actions_div_top",
    getComputedStyle(document.body).getPropertyValue(
      "--actions_div_top_default"
    )
  );
  ...
  document.getElementById("location_description_div").innerText =
    current_location.getDescription();
  ...
  if (location.housing?.is_unlocked) {
    ...
    start_sleeping_div.innerHTML =
      '<i class="material-icons">bed</i>  ' + location.housing.text_to_sleep;
    ...
    open_storage_div.innerHTML =
      '<i class="material-icons">inventory_2</i>  Open your personal chest';
    ...
    if (!dialogues[dialogue].is_unlocked || dialogues[dialogue].is_finished) {
      ...
      Object.keys(dialogues[dialogue].textlines).forEach((line) => {
        if (lines_available) {
          ...
          lines_available =
            dialogues[dialogue].textlines[line].is_unlocked &&
            !dialogues[dialogue].textlines[line].is_finished;
        }
      });
  ...
  if (available_dialogues.length > 0) {
    location_choice_divs["dialogues"] = create_location_choice_dropdown({
      name: "Talk to someone",
      icon: "question_answer",
      class_name: "choice_dialogue",
    });

    location_choice_divs["dialogues"].append(
      ...create_location_choices({ location: location, category: "talk" })
    );
  }
  ...
  const available_traders = location.traders.filter(
    (trader) => traders[trader].is_unlocked && !traders[trader].is_finished
  );

  if (available_traders.length > 0) {
    location_choice_divs["traders"] = create_location_choice_dropdown({
      name: "Visit a merchant",
      icon: "storefront",
      class_name: "choice_trade",
    });

    location_choice_divs["traders"].append(
      ...create_location_choices({ location: location, category: "trade" })
    );
  }
  ...
  const available_jobs = Object.values(location.activities).filter(
    (activity) =>
      activities[activity.activity_name].type === "JOB" &&
      activities[activity.activity_name].is_unlocked &&
      activity.is_unlocked &&
      activities[activity.activity_name].base_skills_names.filter(
        (skill) => !skills[skill].is_unlocked
      ).length == 0
  );

  if (available_jobs.length > 0) {
    location_choice_divs["jobs"] = create_location_choice_dropdown({
      name: "Find work",
      icon: "work_outline",
      class_name: "choice_work",
    });

    location_choice_divs["jobs"].append(
      ...create_location_choices({ location: location, category: "work" })
    );
  }
  ...
  const available_trainings = Object.values(location.activities).filter(
    (activity) =>
  ...
  if (available_trainings.length > 0) {
    location_choice_divs["trainings"] = create_location_choice_dropdown({
      name: "Train",
      icon: "fitness_center",
      class_name: "choice_train",
    });

    location_choice_divs["trainings"].append(
      ...create_location_choices({ location: location, category: "train" })
    );
  }
  ...
  if (global_flags.is_gathering_unlocked) {
    const available_gatherings = Object.values(location.activities).filter(
      (activity) =>
    ...
    if (available_gatherings.length > 0) {
      location_choice_divs["gatherings"] = create_location_choice_dropdown({
        name: "Gather resources",
        icon: "search",
        class_name: "choice_gather",
      });

      location_choice_divs["gatherings"].append(
        ...create_location_choices({ location: location, category: "gather" })
      );
    }
  }

  const available_actions = Object.values(location.actions).filter(
    (action) => action.is_unlocked && !action.is_finished
  );
  if (available_actions.length > 0) {
    location_choice_divs["actions"] = create_location_choice_dropdown({
      name: "Take an action",
      icon: "circle",
      class_name: "choice_action",
    });

    location_choice_divs["actions"].append(
      ...create_location_choices({ location: location, category: "action" })
    );
  }
  ...
  const available_locations = location.connected_locations.filter(
    (loc) =>
      loc.location.is_unlocked &&
      !loc.location.is_finished &&
      !loc.location.is_challenge
  );
  if (available_locations.length > 0) {
    location_choice_divs["locations"] = create_location_choice_dropdown({
      name: "Move somewhere else",
      icon: "directions",
      class_name: "choice_travel",
    });

    location_choice_divs["locations"].append(
      ...create_location_choices({ location: location, category: "travel" })
    );
  }
  ...
  const available_challenges = location.connected_locations.filter(
    (loc) =>
      loc.location.is_challenge &&
      loc.location.is_unlocked &&
      !loc.location.is_finished
  ...
  if (available_challenges.length > 0) {
    location_choice_divs["challenges"] = create_location_choice_dropdown({
      name: "Take on a challenge",
      icon: "warning_amber",
      class_name: "choice_travel",
    });

    location_choice_divs["challenges"].append(
      ...create_location_choices({ location: location, category: "challenge" })
    );
  }
  ...
  const available_fast_travel = [
    ...Object.keys(favourite_locations).filter(
      (key) => key !== current_location.id
    ),
    ...Object.keys(unlocked_beds).filter(
      (key) =>
        key !== location.id &&
        locations[key].is_unlocked &&
        !locations[key].is_finished
    ),
  ];

  if (available_fast_travel.length + (last_combat_location ? 1 : 0) > 0) {
    location_choice_divs["fast_travel"] = create_location_choice_dropdown({
      name: "Fast travel",
      icon: "directions",
      class_name: "choice_travel",
    });

    location_choice_divs["fast_travel"].append(
      ...create_location_choices({
        location: location,
...
function update_location_icon() {
  if (current_location.housing && current_location.housing.is_unlocked) {
    location_icon_span.innerHTML =
      '<i class="material-icons location_bed_icon">bed</i>';
...
function create_location_choices({ location, category, is_combat = false }) {
  ...
  if (category === "talk") {
    for (let i = 0; i < location.dialogues.length; i++) {
      if (
        !dialogues[location.dialogues[i]].is_unlocked ||
        dialogues[location.dialogues[i]].is_finished
      ) {
        //skip if dialogue is not available
        continue;
      }

      const lines_available =
        location.dialogues.filter((dialogue) => {
          let lines_available = false;
          Object.keys(dialogues[dialogue].textlines).forEach((line) => {
            if (lines_available) {
              return;
      ...
      dialogue_div.innerHTML +=
        `<i class="material-icons location_choice_icon">check_box_outline_blank</i> ` +
        dialogues[location.dialogues[i]].starting_text;
      dialogue_div.classList.add("start_dialogue", "location_choice");
      dialogue_div.setAttribute("data-dialogue", location.dialogues[i]);
      dialogue_div.setAttribute(
        "onclick",
        "start_dialogue(this.getAttribute('data-dialogue'));"
      );
    ...
    for (let i = 0; i < location.traders.length; i++) {
      if (
        !traders[location.traders[i]].is_unlocked ||
        traders[location.traders[i]].is_finished
      ) {
        //skip if trader is not available
        continue;
      }
      ...
      trader_div.innerHTML +=
        `<i class="material-icons location_choice_icon">check_box_outline_blank</i> ` +
        traders[location.traders[i]].trade_text;
      trader_div.classList.add("start_trade", "location_choice");
      trader_div.setAttribute("data-trader", location.traders[i]);
      trader_div.setAttribute(
        "onclick",
        "startTrade(this.getAttribute('data-trader'));"
      );
    ...
    Object.keys(location.activities).forEach((key) => {
      if (
        !activities[location.activities[key].activity_name]?.is_unlocked ||
      ...
      activity_div.setAttribute(
        "onclick",
        "start_activity(this.getAttribute('data-activity'));"
      );
      ...
      job_tooltip.innerHTML +=
        `Pays ${format_money(
          location.activities[key].get_payment()
        )} per every ` +
        `${format_working_time(
          location.activities[key].working_period
        )} worked`;
      ...
      activity_div.innerHTML +=
        `<i class="material-icons location_choice_icon">check_box_outline_blank</i> ` +
        location.activities[key].starting_text;
    ...
    Object.keys(location.activities).forEach((key) => {
      if (
        !activities[location.activities[key].activity_name]?.is_unlocked ||
        !location.activities[key]?.is_unlocked ||
        activities[location.activities[key].activity_name].type !==
          "TRAINING" ||
        activities[
          location.activities[key].activity_name
        ].base_skills_names.filter((skill) => !skills[skill].is_unlocked)
          .length > 0
      ) {
        return;
      }
    ...
    Object.keys(location.activities).forEach((key) => {
      ...
      activity_div.appendChild(
        create_gathering_tooltip(location.activities[key])
      );
    ...
    if (!is_combat) {
      for (let i = 0; i < location.connected_locations.length; i++) {
        if (
          !location.connected_locations[i].location.is_unlocked ||
          location.connected_locations[i].location.is_finished
        ) {
          //skip if not unlocked or if finished
          continue;
        }
        ...
        if ("connected_locations" in location.connected_locations[i].location) {
          // check again if connected location is normal or combat
          action.classList.add("travel_normal");
          if ("custom_text" in location.connected_locations[i]) {
            action.innerHTML =
              `<i class="material-icons location_choice_icon">check_box_outline_blank</i> ` +
              location.connected_locations[i].custom_text;
          } else {
            action.innerHTML =
              `<i class="material-icons location_choice_icon">check_box_outline_blank</i> ` +
              "Go to [" +
              location.connected_locations[i].location.name +
              "]";
          }
        } else {
        ...
        action.setAttribute(
          "data-travel",
          location.connected_locations[i].location.id
        );
        action.setAttribute(
          "onclick",
          "change_location(this.getAttribute('data-travel'));"
        );
    ...
    choice_list.sort(
      (a, b) =>
        b.classList.contains("travel_normal") -
        a.classList.contains("travel_normal")
    );
    ...
    for (let i = 0; i < available_challenges.length; i++) {
      ...
      if ("custom_text" in available_challenges[i]) {
        action.innerHTML =
          `<i class="material-icons">warning_amber</i>  ` +
          available_challenges[i].custom_text;
      } else {
        action.innerHTML =
          `<i class="material-icons">warning_amber</i>  ` +
          "Enter the " +
          available_challenges[i].location.name;
      }
    ...
    Object.keys(location.actions).forEach((key) => {
      if (
        location.actions[key].is_finished ||
        !location.actions[key].is_unlocked
      ) {
        return;
      }
      ...
      location_action_div.classList.add(
        "location_action_div",
        "start_location_action",
        "location_choice"
      );
      location_action_div.setAttribute("data-location_action", key);
      location_action_div.setAttribute(
        "onclick",
        "start_location_action(this.getAttribute('data-location_action'));"
      );

      location_action_div.appendChild(
        create_location_action_tooltip(location.actions[key])
      );

      location_action_div.innerHTML +=
        `<i class="material-icons location_choice_icon">check_box_outline_blank</i> ` +
        location.actions[key].starting_text;
...
function create_fast_travel_choices() {
  ...
  if (
    last_combat_location &&
    !available_fast_travel.includes(last_combat_location)
  ) {
  ...
  available_fast_travel = available_fast_travel.sort((a, b) => {
    if (
      locations[a].housing?.is_unlocked &&
      !locations[b].housing?.is_unlocked
    ) {
      return -1;
    } else if (
      !locations[a].housing?.is_unlocked &&
  ...
  for (let i = 0; i < available_fast_travel.length; i++) {
    if (
      !locations[available_fast_travel[i]].is_unlocked ||
      locations[available_fast_travel[i]].is_finished
    ) {
      //skip if not unlocked or if finished
      continue;
    }
    ...
    if (locations[available_fast_travel[i]].tags.safe_zone) {
      ...
      if (locations[available_fast_travel[i]].housing?.is_unlocked) {
        action.innerHTML =
          `<i class="material-icons">bed</i> <span class="fast_travel_name">` +
          "Travel to [" +
          locations[available_fast_travel[i]].name +
          "]</span>";
      } else {
        action.innerHTML =
          `<i class="material-icons location_choice_icon">check_box_outline_blank</i> <span class="fast_travel_name">` +
          "Travel to [" +
          locations[available_fast_travel[i]].name +
          "]</span>";
      }
      ...
      action.setAttribute(
        "onclick",
        "change_location(this.getAttribute('data-travel'), event);"
      );
    } else {
      action.classList.add("travel_combat");

      action.innerHTML = `<i class="material-icons">warning_amber</i> <span class="fast_travel_name">Travel to [${
        locations[available_fast_travel[i]].name
      }]</span>`;
    ...
    if (
      !locations[available_fast_travel[i]].housing?.is_unlocked &&
      locations[available_fast_travel[i]].id !== last_combat_location
    ) {
      ...
      removal_button.setAttribute(
        "onclick",
        "remove_location_from_favourites({location_id:this.parentNode.getAttribute('data-travel')})"
      );
...
function remove_fast_travel_choice({ location_id }) {
  const element = location_choice_divs["fast_travel"].querySelector(
    `[data-travel="${location_id}"`
  );
  if (
    location_id === last_combat_location ||
    locations[location_id].housing?.is_unlocked
  ) {
    //remove only button
    element
      .getElementsByClassName("fast_travel_removal_button")[0]
      .parentNode.remove();
  } else {
    //remove full element
    element.remove();
...
function update_displayed_combat_location(location) {
  ...
  document.documentElement.style.setProperty(
    "--actions_div_height",
    getComputedStyle(document.body).getPropertyValue(
      "--actions_div_height_combat"
    )
  );
  document.documentElement.style.setProperty(
    "--actions_div_top",
    getComputedStyle(document.body).getPropertyValue("--actions_div_top_combat")
  );

  enemy_count_div.children[0].children[1].innerHTML =
    location.enemy_count -
    (location.enemy_groups_killed % location.enemy_count);

  action = create_location_choices({
    location: location,
    category: "travel",
    is_combat: true,
  });
...
function create_location_types_display(current_location) {
  for (let i = 0; i < current_location.types?.length; i++) {
    ...
    type_div.innerHTML =
      current_location.types[i].type +
      (current_location.types[i].stage > 1
        ? ` ${"I".repeat(current_location.types[i].stage)}`
        : "");
    type_div.classList.add("location_type_div");

    const type_tooltip = document.createElement("div");
    type_tooltip.innerHTML =
      location_types[current_location.types[i].type].stages[
        current_location.types[i].stage
      ].description;
    ...
    if (Object.keys(effects || {}).length > 0) {
      ...
      Object.keys(effects).forEach((stat) => {
        if (effects[stat].multiplier) {
          ...
          const actual = get_location_type_penalty(
            type,
            stage,
            stat,
            "multiplier"
          );
          type_tooltip.innerHTML += `<br>${stat_names[stat]} x${
            Math.round(1000 * actual) / 1000
          }`;
          if (base != actual) {
        ...
        if (effects[stat].flat) {
          ...
          type_tooltip.innerHTML += `<br>${stat_names[stat]}: ${
            Math.round(1000 * actual) / 1000
          }`;
          if (base != actual) {
...
function switch_crafting_recipes_subpage(category, subcategory) {
  const elements = document.querySelectorAll(
    `[data-crafting_category='${category}'], [data-crafting_subcategory]`
  );
  for (let i = 0; i < elements.length; i++) {
    if (elements[i].dataset.crafting_subcategory) {
      if (elements[i].dataset.crafting_category === category) {
...
function create_displayed_crafting_recipes() {
  Object.keys(recipes).forEach((recipe_category) => {
    Object.keys(recipes[recipe_category]).forEach((recipe_subcategory) => {
      ...
      Object.keys(recipes[recipe_category][recipe_subcategory]).forEach(
        (recipe) => {
          add_crafting_recipe_to_display({
...
function add_crafting_recipe_to_display({ category, subcategory, recipe_id }) {
  ...
  if (subcategory === "items") {
    ...
    recipe_div.children[0].innerHTML =
      '<i class="material-icons icon" style="visibility:hidden"> keyboard_double_arrow_down </i>' +
      recipe_div.children[0].innerHTML;
    ...
    recipe_div.addEventListener("click", (event) => {
      if (
        event.target.classList.contains("recipe_item_name") &&
        !event.target.parentNode.classList.contains("recipe_unavailable")
      ) {
        window.useRecipe(event.target);
      } else if (event.target.classList.contains("craft_ammount_button")) {
        window.useRecipe(
          event.target.parentNode,
          Number(event.target.dataset.craft_ammount)
        );
      }
    });
    ...
    recipe_div.append(
      create_recipe_tooltip({ category, subcategory, recipe_id })
    );
  } else if (subcategory === "components") {
    recipe_name_span.classList.add("recipe_name");
    recipe_div.children[0].innerHTML =
      '<i class="material-icons icon crafting_dropdown_icon"> keyboard_double_arrow_down </i>' +
      recipe_div.children[0].innerHTML;
    const material_selection = document.createElement("div");
    material_selection.classList.add("folded_material_list");
    recipe_div.addEventListener("click", (event) => {
      if (
        event.target.classList.contains("recipe_name") ||
        event.target.classList.contains("crafting_dropdown_icon")
      ) {
        window.updateDisplayedMaterialChoice({
          category,
          subcategory,
          recipe_id,
        });
    ...
    if (recipe.item_type === "Armor") {
      ...
      console.warn(
        `Recipe "${category}" -> "${subcategory}" -> "${recipe_id}" has wrong type of resulting item ("${recipe.item_type}")`
      );
    }
    ...
    component_selection_1.parentNode.children[0].addEventListener(
      "click",
      (event) => {
        //unfold a list for selection; its content already loaded by a different function
        if (
          event.target.classList.contains("crafting_selection") ||
          event.target.classList.contains("subcrafting_dropdown_icon")
        ) {
          component_selection_1.children[1].classList.toggle(
            "selected_component_list"
          );
          component_selection_1.children[0].classList.toggle(
            "selected_component_category"
          );
          if (
            recipe_div.querySelectorAll(".folded_crafting_selection").item(0)
              .lastChild &&
            !is_element_above_x(
              recipe_div.querySelectorAll(".folded_crafting_selection").item(0)
                .lastChild,
              document.getElementById("exit_crafting_button")
            )
          ) {
            recipe_div
              .querySelectorAll(".folded_crafting_selection")
              .item(0)
    ...
    component_selection_2.parentNode.children[1].addEventListener(
      "click",
      (event) => {
        //unfold a list for selection; its content already loaded by a different function
        if (
          ...
          component_selection_2.children[1].classList.toggle(
            "selected_component_list"
          );
          component_selection_2.children[0].classList.toggle(
            "selected_component_category"
          );
          if (
            !is_element_above_x(
              recipe_div.querySelector(".recipe_creation_button"),
              document.getElementById("exit_crafting_button")
            )
          ) {
            recipe_div
              .querySelector(".recipe_creation_button")
    ...
    accept_recipe_button.innerHTML =
      "<span class='recipe_creation_span'>Create</span>";
    ...
    accept_recipe_button.append(
      create_recipe_tooltip({
        category,
        subcategory,
        recipe_id,
        components: [],
  ...
  Object.keys(recipes).forEach((recipe_category) => {
    Object.keys(recipes[recipe_category]).forEach((recipe_subcategory) => {
      Object.keys(recipes[recipe_category][recipe_subcategory]).forEach(
        (recipe) => {
          if (
            recipes[recipe_category][recipe_subcategory][recipe].is_unlocked
          ) {
            if (
              crafting_pages[recipe_category][recipe_subcategory].querySelector(
                `[data-recipe_id="${recipe}"]`
              )
            ) {
              update_displayed_crafting_recipe({
                category: recipe_category,
                subcategory: recipe_subcategory,
                recipe_id: recipe,
              });
 ...
 */
function update_displayed_crafting_recipe({
  category,
  subcategory,
  recipe_id,
}) {
  const recipe_div = crafting_pages[category][subcategory].querySelector(
 ...
 */
function create_recipe_tooltip({
  category,
  subcategory,
  recipe_id,
  material,
  ...
  if (subcategory === "items") {
    tooltip.innerHTML = create_recipe_tooltip_content({
      category,
      subcategory,
      recipe_id,
    });
    ...
    if (!material) {
      throw new Error(
        `Component recipes require passing a material, but recipe "${category}" -> "${subcategory}" -> "${recipe_id}" had none!`
      );
    }
    tooltip.innerHTML = create_recipe_tooltip_content({
      category,
...
function update_recipe_tooltip({
  category,
  subcategory,
  recipe_id,
  components,
}) {
  const tooltip = crafting_pages[category][subcategory]
    .querySelector(`[data-recipe_id="${recipe_id}"]`)
    .querySelector(`.${subcategory}_recipe_tooltip`);
  const recipe = recipes[category][subcategory][recipe_id];
  if (subcategory === "items") {
    tooltip.innerHTML = create_recipe_tooltip_content({
      category,
    ...
    const material_selections_div = crafting_pages[category][
      subcategory
    ].querySelector(`[data-recipe_id='${recipe_id}']`).children[1];
    for (let i = 0; i < material_selections_div.children.length; i++) {
      ...
      const material_recipe = recipe.materials.filter(
        (material) => material.material_id === id
      );

      material_selections_div.children[i].children[1].innerHTML =
        create_recipe_tooltip_content({
          category,
          subcategory,
          recipe_id,
          material: material_recipe[0],
        });
    }
  } else if (subcategory === "equipment") {
    tooltip.innerHTML = create_recipe_tooltip_content({
...
function create_recipe_tooltip_content({
  category,
  subcategory,
  recipe_id,
  material,
  ...
  if (subcategory === "items") {
    ...
    tooltip += `Success chance: <b><span style="color:${
      success_chance > 74
        ? "lime"
        : success_chance > 49
        ? "yellow"
        ...
        ? "orange"
        : "red"
    }">${success_chance}%</span></b><br><br>Materials required:<br>`;
    for (let i = 0; i < recipe.materials.length; i++) {
      if (recipe.materials[i].material_id) {
        const key =
          item_templates[recipe.materials[i].material_id].getInventoryKey();
        if (character.inventory[key]?.count >= recipe.materials[i].count) {
          tooltip += `<span style="color:lime"><b>${item_templates[
            recipe.materials[i].material_id
          ].getName()} x${character.inventory[key]?.count || 0}/${
            recipe.materials[i].count
          }</b></span><br>`;
        } else {
          tooltip += `<span style="color:red"><b>${item_templates[
            recipe.materials[i].material_id
          ].getName()} x${character.inventory[key]?.count || 0}/${
            recipe.materials[i].count
          }</b></span><br>`;
        ...
        Object.keys(character.inventory).forEach((key) => {
          if (
            character.inventory[key].item.material_type ===
            recipe.materials[i].material_type
          ) {
        ...
        for (let j = 0; j < mats.length; j++) {
          if (mats[j].count >= recipe.materials[i].count) {
            any_available = true;
            mat_list += `<span style="color:lime"><b>${mats[
              j
            ].item.getName()} x${mats[j].count || 0}/${
              recipe.materials[i].count
            }</b></span><br>`;
          } else {
            mat_list += `<span style="color:red"><b>${mats[
              j
            ].item.getName()} x${mats[j].count || 0}/${
              recipe.materials[i].count
            }</b></span><br>`;
    ...
    tooltip += `<br>Result:<br><div class="recipe_result">${create_item_tooltip_content(
      {
        ...
        options: { skip_quality: true, anchor_tooltip: true },
      }
    )}</div>`;
  } else if (
    subcategory === "components" ||
    ...
    if (
      character.inventory[
        item_templates[material.material_id].getInventoryKey()
    ...
    const quality_range = recipe.get_quality_range(
      station_tier - item_templates[material.result_id].component_tier
    );
    const xp_val_1 = get_recipe_xp_value({
      ...
      result_tier: item_templates[material.result_id].component_tier,
    ...
    tooltip += `<br>Result:<br><div class="recipe_result">${create_item_tooltip_content(
      {
        item: item_templates[material.result_id],
    ...
    if (!components) {
      ...
      if (character.inventory[material.material_id]?.count >= material.count) {
        tooltip += `<span style="color:lime"><b>${item_templates[
          ...
          character.inventory[material.material_id]?.count || 0
        }/${material.count}</b></span><br>`;
      } else {
        tooltip += `<span style="color:red"><b>${item_templates[
          material.material_id
      ...
      const quality_range = recipe.get_quality_range(
        recipe.get_component_quality_weighted(),
        station_tier - item_templates[material.result_id].component_tier
      );
      const xp_val_1 = get_recipe_xp_value({
        category,
      ...
      if (recipe.item_type === "Weapon") {
        ...
        throw new Error(
          `Recipe "${category}" -> "${subcategory}" -> "${recipe_id}" has an incorrect item type "${recipe.item_type}"`
        );
      }
      const quality_range = recipe.get_quality_range(
        recipe.get_component_quality_weighted(
        ...
        station_tier -
          Math.max(
            components[0].item.component_tier,
            components[1].item.component_tier
          ) || 0
      );
      const xp_val_1 = get_recipe_xp_value({
        category,
        ...
        recipe_id,
        selected_components: [
          item_templates[components[0].item.id],
          item_templates[components[1].item.id],
        ],
        rarity_multiplier: rarity_multipliers[getItemRarity(quality_range[0])],
      });
      const xp_val_2 = get_recipe_xp_value({
      ...
      tooltip += `<br>XP value: ${xp_val_1} - ${xp_val_2}<br>`;
      tooltip += `Result:<br><div class="recipe_result">${create_item_tooltip_content(
        { item, options: { quality: quality_range } }
      )}</div>`;
    } else {
      throw new Error(
        `Somehow recipe "${category}" -> "${subcategory}" -> "${recipe_id}" received more components than there should be (${components.length} instead of 2)`
      );
    }
  } else {
    console.error(`No such crafting subcategory as "${subcategory}"`);
 ...
 */
function update_displayed_component_choice({
  category,
  recipe_id,
  component_keys = {},
}) {
  const recipe_div = crafting_pages[category]["equipment"].querySelector(
    `[data-recipe_id="${recipe_id}"]`
  );
  const recipe = recipes[category]["equipment"][recipe_id];

  const component_selections_div = crafting_pages[category][
    "equipment"
  ].querySelector(`[data-recipe_id='${recipe_id}']`).children[1].children;
  ...
  const components = [];
  components.push(
    Object.values(character.inventory).filter((item) => {
  ...
  for (let i = 0; i < 2; i++) {
    for (let j = 0; j < components[i].length; j++) {
      ...
      item_div.appendChild(
        create_item_tooltip(components[i][j].item, {
          class_name: "recipe_tooltip",
        })
      );
      ...
      item_div.addEventListener("click", () => {
        toggle_exclusive_class({
          element: item_div,
          siblings_only: true,
          class_name: "selected_component",
        });
        const components = [];
        const component_1_key =
          recipe_div.children[1].children[0].children[1].querySelector(
            ".selected_component"
          )?.dataset.item_key;
        if (component_1_key) {
        ...
        const component_2_key =
          recipe_div.children[1].children[1].children[1].querySelector(
            ".selected_component"
          )?.dataset.item_key;
        if (component_2_key) {
  ...
  for (let i = 0; i < 2; i++) {
    [...component_selections_div[i].children[1].children]
      .sort((a, b) => {
        if (
          Number.parseInt(a.dataset.component_tier) >
          Number.parseInt(b.dataset.component_tier)
        ) {
          return -1;
        } else if (
          Number.parseInt(a.dataset.component_tier) <
      ...
      .forEach((node) =>
        component_selections_div[i].children[1].appendChild(node)
      );
  }
}
 ...
 */
function update_displayed_material_choice({
  category,
  subcategory,
  recipe_id,
  refreshing,
  ...
  const materials = Object.values(character.inventory).filter((item) => {
    return (
      recipe.materials.filter(
        (material) => material.material_id === item.item?.id
      ).length > 0
    );
  });
  ...
  for (let i = 0; i < materials.length; i++) {
    const material_recipe = recipe.materials.filter(
      (material) => material.material_id === materials[i].item.id
    )[0];
    const item_div = document.createElement("div");
    const name_span = document.createElement("span");
    name_span.innerHTML = `<i class="material-icons icon selected_material_icon"> check </i>${item_templates[
      material_recipe.result_id
    ].getName()}`;
    ...
    Object.keys(recipes[recipe_category]).forEach((recipe_subcategory) => {
      ...
      Object.keys(recipes[recipe_category][recipe_subcategory]).forEach(
        (recipe) => {
          ...
          const recipe_div = crafting_pages[recipe_category][
            recipe_subcategory
          ].querySelector(`[data-recipe_id="${recipe}"`);
          if (
            !recipes[recipe_category][recipe_subcategory][
              recipe
            ].get_availability().available_ammount
          ) {
            recipe_div.classList.add("recipe_unavailable");
          } else {
            recipe_div.classList.remove("recipe_unavailable");
...
function create_gathering_tooltip(location_activity) {
  ...
  const { gathering_time_needed, gained_resources } =
    location_activity.getActivityEfficiency();

  let skill_names = "";
  for (
    let i = 0;
    i < activities[location_activity.activity_name].base_skills_names.length;
    i++
  ) {
    skill_names +=
      skills[
        activities[location_activity.activity_name].base_skills_names[i]
      ].name();
  }
  ...
  gathering_tooltip.innerHTML += `Every ${format_time({
    time: { minutes: gathering_time_needed },
    round: false,
  })}, chance to find:`;
  for (let i = 0; i < gained_resources.length; i++) {
    gathering_tooltip.innerHTML += `<br>x${
      gained_resources[i].count[0] === gained_resources[i].count[1]
        ? gained_resources[i].count[0]
        : `${gained_resources[i].count[0]}-${gained_resources[i].count[1]}`
    } "${gained_resources[i].name}" at ${Math.round(
      100 * gained_resources[i].chance
    )}%`;
  }
...
function update_gathering_tooltip(activity) {
  ...
  if (parent) {
    ...
    gathering_tooltip = document
      .getElementById("gathering_progress_bar_max")
      ?.querySelector(`[data-job_tooltip="${activity.activity_id}"]`);
  }
  ...
  const { gathering_time_needed, gained_resources } =
    activity.getActivityEfficiency();

  let skill_names = "";
  for (
    let i = 0;
    i < activities[activity.activity_name].base_skills_names.length;
    i++
  ) {
    skill_names +=
      skills[activities[activity.activity_name].base_skills_names[i]].name();
  }
  ...
  gathering_tooltip.innerHTML += `Every ${format_working_time(
...
function update_displayed_health() {
  ...
  current_health_value_div.innerText =
    Math.ceil(character.stats.full.health) +
    "/" +
    Math.ceil(character.stats.full.max_health) +
    " hp";
...
function update_displayed_stamina() {
  //call it when eating, resting or fighting
  current_stamina_value_div.innerText =
    Math.round(character.stats.full.stamina) +
    "/" +
    Math.round(character.stats.full.max_stamina) +
    " stamina";
  current_stamina_bar.style.width =
    (
      (character.stats.full.stamina * 100) /
      character.stats.full.max_stamina
    ).toString() + "%";
}
...
function update_displayed_stats() {
  ...
  Object.keys(stats_divs).forEach(function (key) {
    if (key === "crit_rate" || key === "crit_multiplier") {
      stats_divs[key].innerHTML = `${(character.stats.full[key] * 100).toFixed(
        1
      )}%`;
  ...
  if (
    character.equipment["off-hand"] != null &&
    character.equipment["off-hand"].offhand_type === "shield"
  ) {
    //HAS SHIELD
  ...
  attack_stats.children[1].innerHTML = `Atk spd: ${
    Math.round(character.get_attack_speed() * 100) / 100
  }`;
  attack_stats.children[2].innerHTML = `AP:  ${Math.round(ap)}`;
  attack_stats.children[4].innerHTML = `Def: ${Math.round(
    character.stats.full.defense
  )} `;
}
...
function update_stat_description(stat) {
  ...
  if (stat === "attack_power") {
    target.innerHTML = `<br>Breakdown:
        <br>Base value (weapon * str/10): ${
          Math.round(100 * character.stats.total_flat.attack_power) / 100
        }`;
  } else if (stat === "attack_points") {
    target.innerHTML = `<br>Breakdown:
        <br>Base value: ${
          Math.round(100 * character.stats.total_flat.attack_points) / 100
        }`;
    ...
    target.innerHTML = `<br>Breakdown:
            <br>Base value: ${
              Math.round(100 * character.stats.total_flat[stat]) / 100
            }`;
  } else {
    target.innerHTML = `<br>Breakdown:
  ...
  Object.keys(character.stats.flat).forEach((stat_type) => {
    if (
      character.stats.flat[stat_type][stat] &&
      character.stats.flat[stat_type][stat] !== 0
    ) {
      target.innerHTML += `<br>${capitalize_first_letter(
        stat_type.replace("_", " ")
      )}: +${Math.round(100 * character.stats.flat[stat_type][stat]) / 100}`;
    }
  });

  Object.keys(character.stats.multiplier).forEach((stat_type) => {
    if (
      character.stats.multiplier[stat_type][stat] &&
      character.stats.multiplier[stat_type][stat] !== 1
    ) {
      target.innerHTML += `<br>${capitalize_first_letter(
        stat_type.replace("_", " ")
      )}: x${
        Math.round(100 * character.stats.multiplier[stat_type][stat]) / 100
      }`;
    }
  });
...
function update_displayed_effects() {
  ...
  if (effect_count > 0) {
    ...
    Object.values(active_effects).forEach((effect) => {
      effect_divs[effect.name] = create_effect_tooltip({
...
function update_displayed_effect_durations() {
  Object.keys(effect_divs).forEach((key) => {
    if (!active_effects[key]?.duration) {
      ...
      effect_divs[key].querySelector(".active_effect_duration").innerHTML =
        format_time({
          time: { minutes: active_effects[key].duration },
          round: false,
        });
    }
  });
...
function update_displayed_time() {
  if (is_night(current_game_time)) {
    time_field.innerHTML =
      current_game_time.toString() +
      '<span class="material-icons icon icon_night">nightlight_round</span>';
  } else {
    time_field.innerHTML =
      current_game_time.toString() +
      '<span class="material-icons icon icon_day">light_mode</span>';
  }
}
...
function format_money(num) {
  ...
  if (num > 0) {
    value =
      num % 10 != 0 ? `${num % 10}<span class="coin coin_wood">W</span>` : "";

    if (num > 9) {
      value =
        (Math.floor(num / 10) % 100 != 0
          ? `${
              Math.floor(num / 10) % 100
            }<span class="coin coin_copper">C</span>${value ? " " : ""}`
          : "") + value;
      if (num > 999) {
        value =
          (Math.floor(num / 1000) % 100 != 0
            ? `${
                Math.floor(num / 1000) % 100
              }<span class="coin coin_silver">S</span>${value ? " " : ""}`
            : "") + value;
        if (num > 99999) {
          value =
            `${Math.floor(num / 100000)}<span class="coin coin_gold">G</span>${
              value ? " " : ""
            }` + value;
        }
      }
...
function update_displayed_character_xp(did_level = false) {
  /*
    character_xp_div
        ...
        charaxter_xp_value
    */
  character_xp_div.children[0].children[0].style.width = `${
    (100 * character.xp.current_xp) / character.xp.xp_to_next_lvl
  }%`;
  character_xp_div.children[1].innerText = `${Math.floor(
    character.xp.current_xp
  )}/${Math.ceil(character.xp.xp_to_next_lvl)} xp`;
...
function update_displayed_stamina_efficiency() {
  data_entry_divs.stamina.innerHTML = `<span class="data_entry_name">Stamina efficiency:</span><span class="data_entry_value">${
    Math.round(100 * character.stats.full.stamina_efficiency) / 100 || 1
  }</span>`;
}
...
function update_displayed_dialogue(dialogue_key) {
  ...
  Object.keys(dialogue.textlines).forEach(function (key) {
    //add buttons for textlines
    if (
      dialogue.textlines[key].is_unlocked &&
      !dialogue.textlines[key].is_finished
    ) {
      //do only if text_line is not unavailable
      if (dialogue.textlines[key].required_flags) {
        if (
          (dialogue.textlines[key].required_flags.yes &&
            !Array.isArray(dialogue.textlines[key].required_flags.yes)) ||
          (dialogue.textlines[key].required_flags.no &&
            !Array.isArray(dialogue.textlines[key].required_flags.no))
        ) {
          console.error(
            `Textline "${key}" in dialogue "${dialogue_key}" has required flag passed as a single value but it should be an array!`
          );
        }
        if (dialogue.textlines[key].required_flags.yes) {
          for (
            let i = 0;
            i < dialogue.textlines[key].required_flags.yes.length;
            i++
          ) {
        ...
        if (dialogue.textlines[key].required_flags.no) {
          for (
            let i = 0;
            i < dialogue.textlines[key].required_flags.no.length;
            i++
          ) {
      ...
      textline_div.setAttribute(
        "onclick",
        `start_textline(this.getAttribute('data-textline'))`
      );
  ...
  end_dialogue_div.innerHTML =
    "<i class='material-icons'>arrow_back</i> " + dialogue.ending_text;
...
function update_displayed_textline_answer({ text, is_description }) {
  if (is_description) {
    ...
    document.getElementById("dialogue_answer_div").innerHTML =
      '<i>"' + text + '"</i>';
  }
}
...
function start_activity_display(current_activity) {
  ...
  action_status_div.innerText =
    activities[current_activity.activity_name].action_text;
  ...
  if (activities[current_activity.activity_name].base_skills_names) {
    const percent_xp =
      get_total_skill_level(
        activities[current_activity.activity_name].base_skills_names
      ) ==
      skills[activities[current_activity.activity_name].base_skills_names]
        .max_level
        ? "Max"
        : `${
            Math.round(
    ...
    if (curr_xp !== "Max") {
      action_xp_div.innerText += ` ${skills[
        activities[current_activity.activity_name].base_skills_names
      ].name()} (${percent_xp}  [${curr_xp}/${needed_xp}])`;
    } else {
      action_xp_div.innerText += ` ${skills[
        activities[current_activity.activity_name].base_skills_names
      ].name()} (Maxxed out!)`;
    ...
    if (activities[current_activity.activity_name].type !== "GATHERING") {
      const time_needed = Math.ceil(
        (needed_xp - curr_xp) /
          (current_activity.skill_xp_per_tick *
            get_skill_xp_gain(
              skills[
                activities[current_activity.activity_name].base_skills_names
              ].skill_id
      ...
      if (!isNaN(time_needed)) {
        action_xp_div.innerHTML += `<br>Next level in ${format_reading_time(
          time_needed
        )}`;
      }
    } else {
      const time_needed = Math.ceil(
        (current_activity.gathering_time_needed * (needed_xp - curr_xp)) /
          (current_activity.skill_xp_per_tick *
            get_skill_xp_gain(
              skills[
                activities[current_activity.activity_name].base_skills_names
    ...
    console.warn(
      `Activity "${current_activity.activity_name}" has no skills assigned!`
    );
  }
  ...
  if (current_activity.gained_resources) {
    ...
    action_progress_bar.style.width =
      (385 * current_activity.gathering_time) /
        current_activity.gathering_time_needed +
      "px";
    action_progress_bar_max.id = "gathering_progress_bar_max";
    action_div.appendChild(action_progress_bar_max);
    action_progress_bar_max.appendChild(
      create_gathering_tooltip(current_activity)
    );
  }
  ...
  if (activities[current_activity.activity_name].type === "JOB") {
    ...
    if (!enough_time_for_earnings(current_activity)) {
      ...
      time_info_div.innerHTML = `Next earnings in: ${format_working_time(
        current_activity.working_period - current_activity.working_time
      )}`;
    }
...
function update_displayed_ongoing_activity(current_activity, is_job) {
  if (is_job) {
    document.getElementById(
      "action_end_earnings"
    ).innerHTML = `(earnings: ${format_money(current_activity.earnings)})`;
    ...
    if (!enough_time_for_earnings(current_activity)) {
      ...
      time_info_div.innerHTML = `Next earnings in: ${format_working_time(
        current_activity.working_period -
          (current_activity.working_time % current_activity.working_period)
      )}`;
    }
  }
  ...
  if (!action_xp_div) {
    console.warn(
      `Failed to find htmlElement with id "action_xp_div" for activity "${current_activity.activity_id}"`
    );
    return;
  }
  ...
  if (current_activity.gained_resources) {
    document.getElementById("gathering_progress_bar").style.width =
      (385 * current_activity.gathering_time) /
        current_activity.gathering_time_needed +
      "px";
  }
...
function update_location_action_progress_bar(percent) {
  document.getElementById("action_progress_bar").style.width =
    385 * percent + "px";
}
...
function start_reading_display(title) {
  ...
  const action_status_div = document.createElement("div");
  action_status_div.innerText = `Reading the book, ${format_reading_time(
    item_templates[title].getRemainingTime()
  )} left`;
  action_status_div.id = "action_status_div";
...
function create_new_skill_bar(skill) {
  ...
  if (skill_bar_divs[skill.category][skill.skill_id]) {
    console.trace(
      `Tried to create a skillbar for skill "${skill.skill_id}", but it already has one!`
    );
    return;
  }
  skill_bar_divs[skill.category][skill.skill_id] =
  ...
  skill_bar_divs[skill.category][skill.skill_id].setAttribute(
    "data-skill",
    skill.skill_id
  );
  skill_bar_divs[skill.category][skill.skill_id].classList.add("skill_div");
  skill_list
    .querySelector(`[data-skill_category=${skill.category}]`)
    .querySelector("[data-skill_category_skills]")
    .appendChild(skill_bar_divs[skill.category][skill.skill_id]);
...
function update_displayed_skill_bar(skill, leveled_up = true) {
  ...
  if (skill.current_xp !== "Max") {
    skill_bar_divs[skill.category][
      ...
      (100 * Math.floor((skill.current_xp / skill.xp_to_next_lvl) * 1000)) /
      1000
    }%`;
    skill_bar_divs[skill.category][
      skill.skill_id
    ].children[0].children[2].children[0].innerHTML = `${expo(
      skill.current_xp
    )}/${expo(skill.xp_to_next_lvl)}`;
  } else {
    skill_bar_divs[skill.category][
      skill.skill_id
    ].children[0].children[0].children[1].innerHTML = `Max!`;
    skill_bar_divs[skill.category][
      skill.skill_id
    ].children[0].children[2].children[0].innerHTML = `Maxed out!`;
  }
  //skill_bar_xp && tooltip_xp

  skill_bar_divs[skill.category][
    skill.skill_id
  ].children[0].children[1].style.width = `${
    (100 * skill.current_xp) / skill.xp_to_next_lvl
  }%`;
  //skill_bar_current
  ...
  if (get_unlocked_skill_rewards(skill.skill_id)) {
    skill_bar_divs[skill.category][
      skill.skill_id
    ].children[0].children[2].children[4].innerHTML = `<br>${get_unlocked_skill_rewards(
      skill.skill_id
    )}`;
  }
  ...
  if (typeof get_next_skill_milestone(skill.skill_id) !== "undefined") {
    skill_bar_divs[skill.category][
      skill.skill_id
    ].children[0].children[2].children[5].innerHTML = `lvl ${get_next_skill_milestone(
      skill.skill_id
    )}: ???`;
  } else {
    skill_bar_divs[skill.category][
      skill.skill_id
    ].children[0].children[2].children[5].innerHTML = "";
  }

  if (typeof skill.get_effect_description !== "undefined") {
    skill_bar_divs[skill.category][
      skill.skill_id
    ].children[0].children[2].children[3].innerHTML = `${skill.get_effect_description()}`;
    //tooltip_effect
  }
...
function update_displayed_skill_level(skill) {
  ...
  skill_bar_divs[skill.category][
    skill.skill_id
  ].children[0].children[0].children[0].innerHTML = `${skill.name()} : level ${
    skill.current_level
  }/${skill.max_level}`;
  ...
  if (bonus != 0) {
    if (bonus > 0) {
      skill_bar_divs[skill.category][
        skill.skill_id
      ].children[0].children[0].children[0].innerHTML += ` <b>[+${bonus}]</b>`;
    } else {
      skill_bar_divs[skill.category][
        skill.skill_id
      ].children[0].children[0].children[0].innerHTML += ` <b>[${bonus}]</b>`;
...
function update_displayed_skill_xp_gain(skill) {
  ...
  const xp_gain =
    Math.round(
      100 * skill.get_parent_xp_multiplier() * get_skill_xp_gain(skill.skill_id)
    ) / 100 || 1;
  skill_bar_divs[skill.category][
    skill.skill_id
  ].children[0].children[2].children[1].innerHTML = `XP gain: x${xp_gain}<br><span>XP cost scaling: x${skill.xp_scaling}</span>`;
}
...
function sort_displayed_skills({ sort_by = "name", change_direction = false }) {
  ...
  for (let i = 0; i < skill_list.children.length; i++) {
    [
      ...skill_list.children[i].querySelector("[data-skill_category_skills")
        .children,
    ]
      .sort((a, b) => {
        let elem_a;
...
function update_displayed_stance_list(stances, current_stance, fav_stances) {
  ...
  stance_list.innerHTML = `<tr class="stance_list_entry stance_list_header">
  ...
  Object.keys(stances).forEach((stance) => {
    if (stances[stance].is_unlocked) {
      ...
      const stance_info = `<td class="stances_name"><label for="stances_select_${stance}">${stances[stance].name}</td>`;
  ...
  [...stance_list.children]
    .sort((a, b) => {
      ...
      if (
        !stance_a ||
        !stance_b ||
        !stance_a.is_unlocked ||
        !stance_b.is_unlocked
      ) {
        console.error(
          `No such stance as either '${stance_a}' or '${stance_b}', or at least one of them is not yet unlocked!`
        );
      }
...
function create_stance_tooltip(stance) {
  ...
  tooltip_div.innerHTML = `<div>${stance.name}</div><br>
    <div>${stance.getDescription()}</div><br>
    <div>Stamina cost: ${stance.stamina_cost}</div>
    <div class='stance_tooltip_stats'>${create_stance_tooltip_stats(
      stance
    )}</div`;
  ...
  if (target_count > 1 && stance.related_skill) {
    target_count =
      target_count +
      Math.round(
        (target_count * get_total_skill_level(stance.related_skill)) /
          skills[stance.related_skill].max_level
      );
  }

  if (target_count > 1) {
    tooltip_div.innerHTML += `
        <br><div class='stance_tooltip_hitcount'>${
          stance.randomize_target_count ? "Randomly hits up to" : "Hits up to"
        } ${target_count} enemies</div>`;
  }
...
function update_stance_tooltip(stance) {
  stance_bar_divs[stance.id].querySelector(".stance_tooltip_stats").innerHTML =
    create_stance_tooltip_stats(stance);
  ...
  if (target_count > 1) {
    ...
    stance_bar_divs[stance.id].querySelector(
      ".stance_tooltip_hitcount"
    ).innerHTML = `${
      stance.randomize_target_count ? "Randomly hits up to" : "Hits up to"
    } ${target_count} enemies</div>`;
  }
...
function update_displayed_stance(stance) {
  ...
  document.getElementById("character_stance_name").children[0].innerHTML =
    stance.name;
  ...
  if (selection.children) {
    if (selection.querySelector(`[data-stance='${stance.id}']`)) {
      ...
      !faved_stances[stance.id] &&
      selection.querySelector("[data-stance] :checked")
    ) {
...
function update_displayed_faved_stances(stances) {
  ...
  Object.keys(faved_stances).forEach((stance) => {
    ...
    const node = `<div data-stance="${stance}"><input type="radio" id="stances_quick_select_${stance}" name="stance_quick_selection" onclick="select_stance('${stance}')">
  ...
  if (
    selection.children &&
    selection.querySelector(`[data-stance='${selected_stance}']`)
  ) {
    selection.querySelector(
      `[data-stance='${selected_stance}']`
    ).children[0].checked = true;
  }
}
...
function create_new_bestiary_entry(enemy_name) {
  ...
  stat_4_value.innerHTML = `${Math.round(
    enemy.stats.dexterity * Math.sqrt(enemy.stats.intuition || 1)
  )}`;
  ...
  stat_5_value.innerHTML = `${Math.round(
    enemy.stats.agility * Math.sqrt(enemy.stats.intuition || 1)
  )}`;
  ...
  for (let i = 0; i < enemy.loot_list.length; i++) {
    ...
    loot_chance_current.innerHTML = `${
      Math.round(
        10000 * enemy.loot_list[i].chance * enemy.get_droprate_modifier()
      ) / 100
    }%`;
    loot_chance.append(loot_chance_current, loot_chance_base);
    loot_line.append(loot_name, loot_chance);
  ...
  [...bestiary_list.children]
    .sort(
      (a, b) =>
        parseInt(a.getAttribute("data-bestiary")) -
        parseInt(b.getAttribute("data-bestiary"))
    )
...
function update_bestiary_entry(enemy_name) {
  bestiary_entry_divs[enemy_name].children[1].innerHTML =
    enemy_killcount[enemy_name];
}
...
function update_enemy_attack_bar(enemy_id, num) {
  enemies_div.children[enemy_id].querySelector(
    ".enemy_attack_bar"
  ).style.width = `${Math.min(num * 100, 100)}%`;
}
...
function update_backup_load_button(date_string) {
  if (date_string) {
    backup_load_button.innerText = `Load the backup autosave [${date_string.replaceAll(
      "_",
      ":"
    )}]`;
...
function update_other_save_load_button(date_string, is_dev) {
  ...
  if (date_string !== undefined) {
    ...
    if (date_string) {
      other_save_load_button.innerText += ` [${date_string.replaceAll(
        "_",
        ":"
      )}]`;
    } else {
 ...
 */
function toggle_exclusive_class({
  element,
  siblings_only = false,
  class_name,
}) {
  const elems = siblings_only
    ? element.parentNode.querySelectorAll(`.${class_name}`)
