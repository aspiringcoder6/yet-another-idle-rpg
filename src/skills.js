...
class Skill {
  constructor({
    ...
    this.visibility_treshold =
      visibility_treshold < base_xp_cost ? visibility_treshold : base_xp_cost;
    ...
    if (!category) {
      console.warn(
        `Skill "${this.skill_id}" has no category defined and was defaulted to miscellaneous`
      );
    ...
    if (parent_skill) {
      if (skills[parent_skill]) {
        ...
        throw new Error(
          `Skill "${parent_skill}" doesn't exist, so it can't be set as a parent skill`
        );
      }
    }
  ...
  add_xp({ xp_to_add = 0 }) {
    ...
    if (this.current_level < this.max_level) {
      //not max lvl
      if (
        Math.round(100 * (xp_to_add + this.current_xp)) / 100 <
        this.xp_to_next_lvl
      ) {
        // no levelup
        ...
        while (this.total_xp >= this.total_xp_to_next_lvl) {
          level_after_xp += 1;
          this.total_xp_to_next_lvl =
            Math.round(
              (100 *
                this.base_xp_cost *
                (1 - this.xp_scaling ** (level_after_xp + 1))) /
                (1 - this.xp_scaling)
            ) / 100;

          if (this.milestones[level_after_xp]?.unlocks?.skills) {
            unlocks.skills.push(
              ...this.milestones[level_after_xp].unlocks.skills
            );
          }
          if (this.milestones[level_after_xp]?.unlocks?.recipes) {
            unlocks.recipes.push(
              ...this.milestones[level_after_xp].unlocks.recipes
            );
          }
        } //calculates lvl reached after adding xp
        //probably could be done much more efficiently, but it shouldn't be a problem anyway

        let total_xp_to_previous_lvl =
          Math.round(
            (100 *
              this.base_xp_cost *
              (1 - this.xp_scaling ** level_after_xp)) /
              (1 - this.xp_scaling)
          ) / 100;
        //xp needed for current lvl, same formula but for n-1

        if (level_after_xp == 0) {
          //this was for an older issue that seems to have been long fixed
          console.warn(
            `Something went wrong, calculated level of skill "${this.skill_id}" after a levelup was 0.` +
        ...
        let gains;
        if (level_after_xp < this.max_level) {
          //wont reach max lvl
          gains = this.get_bonus_stats(level_after_xp);
          this.xp_to_next_lvl =
            Math.round(
              100 * (this.total_xp_to_next_lvl - total_xp_to_previous_lvl)
            ) / 100;
          this.current_level = level_after_xp;
          this.current_xp =
            Math.round(100 * (this.total_xp - total_xp_to_previous_lvl)) / 100;
        } else {
          //will reach max lvl
        ...
        skill_name =
          skill_name === unknown_skill_name ? this.name() : skill_name;
        ...
        if (
          Object.keys(gains.stats).length > 0 ||
          Object.keys(gains.xp_multipliers).length > 0
        ) {
          ...
          if (gains.stats) {
            Object.keys(gains.stats).forEach((stat) => {
              if (gains.stats[stat].flat) {
                message += `<br> +${gains.stats[stat].flat} ${stat_names[
                  stat
                ].replace("_", " ")}`;
              }
              if (gains.stats[stat].multiplier) {
                message += `<br> x${
                  Math.round(100 * gains.stats[stat].multiplier) / 100
                } ${stat_names[stat].replace("_", " ")}`;
              }
            });
          }
          ...
          if (gains.xp_multipliers) {
            Object.keys(gains.xp_multipliers).forEach((xp_multiplier) => {
              ...
              if (
                xp_multiplier !== "all" &&
                xp_multiplier !== "hero" &&
                xp_multiplier !== "all_skill" &&
                !xp_multiplier.includes("category_")
              ) {
                name = skills[xp_multiplier].name();
                if (!skills[xp_multiplier]) {
                  console.warn(
                    `Skill ${this.skill_id} tried to reward an xp multiplier for something that doesn't exist: ${xp_multiplier}. I could be a misspelled skill name`
                  );
                }
              } else {
              ...
              if (xp_multiplier.includes("category_")) {
                message += `<br> x${
                  Math.round(100 * gains.xp_multipliers[xp_multiplier]) / 100
                } ${name.replace("category_", "")} skills xp gain`;
              } else {
                message += `<br> x${
                  Math.round(100 * gains.xp_multipliers[xp_multiplier]) / 100
                } ${name} xp gain`;
      ...
      if (this.milestones[i]) {
        ...
        if (stats) {
          Object.keys(stats).forEach((stat) => {
            ...
            if (stats[stat].flat) {
              gains.stats[stat].flat =
                (gains.stats[stat].flat || 0) + stats[stat].flat;
            }
            if (stats[stat].multiplier) {
              gains.stats[stat].multiplier =
                (gains.stats[stat].multiplier || 1) * stats[stat].multiplier;
            }
          });
        }
        ...
        if (xp_multipliers) {
          Object.keys(xp_multipliers).forEach((multiplier_key) => {
            gains.xp_multipliers[multiplier_key] =
              (gains.xp_multipliers[multiplier_key] || 1) *
              xp_multipliers[multiplier_key];
            if (which_skills_affect_skill[multiplier_key]) {
              if (
                !which_skills_affect_skill[multiplier_key].includes(
                  this.skill_id
                )
              ) {
  ...
  get_coefficient({ scaling_type, skill_level }) {
    //starts from 1
    ...
    switch (scaling_type) {
      case "flat":
        return (
          1 +
          Math.round(
            (((this.max_level_coefficient - 1) *
              (skill_level || this.current_level)) /
              this.max_level) *
              1000
          ) /
            1000
      ...
      case "multiplicative":
        return (
          Math.round(
            Math.pow(
              this.max_level_coefficient,
              (skill_level || this.current_level) / this.max_level
            ) * 1000
          ) / 1000
        );
  ...
  get_level_bonus(level) {
    //starts from 0
    return (
      (this.max_level_bonus * (level || this.current_level)) / this.max_level
    );
  }
  get_parent_xp_multiplier() {
    if (this.parent_skill) {
      return (
        ...
        Math.max(
          0,
          skills[this.parent_skill].current_level - this.current_level
        )
      );
    } else {
      return 1;
...
function get_unlocked_skill_rewards(skill_id) {
  ...
  const milestones = Object.keys(skill.milestones).filter(
    (level) => level <= skill.current_level
  );
  if (milestones.length > 0) {
    unlocked_rewards = `lvl ${milestones[0]}: ${format_skill_rewards(
      skill.milestones[milestones[0]]
    )}`;
    for (let i = 1; i < milestones.length; i++) {
      unlocked_rewards += `<br>\n\nlvl ${milestones[i]}: ${format_skill_rewards(
        skill.milestones[milestones[i]]
      )}`;
    }
  } else {
    //no rewards
...
function format_skill_rewards(milestone) {
  ...
  if (milestone.xp_multipliers) {
    ...
    if (formatted) {
      formatted += `, x${
        milestone.xp_multipliers[xp_multipliers[0]]
      } ${name} xp gain`;
    } else {
      formatted = `x${
        milestone.xp_multipliers[xp_multipliers[0]]
    ...
    for (let i = 1; i < xp_multipliers.length; i++) {
      ...
      formatted += `, x${
        milestone.xp_multipliers[xp_multipliers[i]]
      } ${name} xp gain`;
    }
  }
  if (milestone.unlocks) {
    ...
    if (milestone.unlocks.recipes) {
      const phrasing =
        milestone.unlocks.recipes.length > 1 ? "new recipes" : "a new recipe";
      if (formatted) {
...
(function () {
  ...
  skills["Pest killer"] = new Skill({
    ...
    description:
      "Small enemies might not seem very dangerous, but it's not that easy to hit them!",
    max_level_coefficient: 2,
    category: "Combat",
    base_xp_cost: 100,
    get_effect_description: () => {
      return `Multiplies AP against small-type enemies by ${
        Math.round(
          get_total_skill_coefficient({
            skill_id: "Pest killer",
            scaling_type: "multiplicative",
  ...
  skills["Giant slayer"] = new Skill({
    ...
    description:
      "Large opponents might seem scary, but just don't get hit and you should be fine!",
    max_level_coefficient: 2,
    category: "Combat",
    get_effect_description: () => {
      return `Multiplies EP against large-type enemies by ${
        Math.round(
          get_total_skill_coefficient({
            skill_id: "Giant slayer",
            scaling_type: "multiplicative",
  ...
  skills["Shield blocking"] = new Skill({
    ...
    category: "Combat",
    get_effect_description: () => {
      return `Increases block chance by flat ${
        Math.round(get_total_level_bonus("Shield blocking") * 1000) / 10
      }%. Increases blocked damage by ${
        Math.round(get_total_level_bonus("Shield blocking") * 5000) / 10
      }%`;
    },
  });
  ...
  skills["Unarmed"] = new Skill({
    ...
    description:
      "It's definitely, unquestionably, undoubtedly better to just use a weapon instead of doing this. But sure, why not?",
    category: "Combat",
    get_effect_description: () => {
      return `Multiplies damage dealt in unarmed combat by ${
        Math.round(
          get_total_skill_coefficient({
            skill_id: "Unarmed",
            scaling_type: "multiplicative",
...
Multiplies attack speed, EP and AP in unarmed combat by ${
        Math.round(
          get_total_skill_coefficient({
            skill_id: "Unarmed",
            scaling_type: "multiplicative",
...
(function () {
  ...
  skills["Quick steps"] = new Skill({
    skill_id: "Quick steps",
    names: { 0: "Quick steps" },
    parent_skill: "Stance mastery",
    description:
      "A swift and precise technique that abandons strength in favor of greater speed",
    max_level_coefficient: 2,
    base_xp_cost: 60,
    category: "Stance",
    max_level: 30,
  ...
  skills["Heavy strike"] = new Skill({
    ...
    parent_skill: "Stance mastery",
    description:
      "A powerful and dangerous technique that abandons speed in favor of overwhelmingly strong attacks",
    max_level_coefficient: 2,
    base_xp_cost: 60,
    category: "Stance",
    max_level: 30,
  ...
  skills["Wide swing"] = new Skill({
    ...
    parent_skill: "Stance mastery",
    description:
      "A special technique that allows striking multiple enemies at once, although at a cost of lower damage",
    max_level_coefficient: 2,
    base_xp_cost: 60,
    category: "Stance",
    max_level: 30,
  ...
  skills["Defensive measures"] = new Skill({
    skill_id: "Defensive measures",
    names: { 0: "Defensive measures" },
    parent_skill: "Stance mastery",
    description:
      "A careful technique focused much more on defense than on attacking",
    max_level_coefficient: 2,
    base_xp_cost: 60,
    category: "Stance",
    max_level: 30,
  ...
  skills["Berserker's stride"] = new Skill({
    skill_id: "Berserker's stride",
    names: { 0: "Berserker's stride" },
    parent_skill: "Stance mastery",
    description:
      "A wild and dangerous technique that focuses on dealing as much damage as possible, while completely ignoring own defense",
    max_level_coefficient: 2,
    base_xp_cost: 60,
    category: "Stance",
    max_level: 30,
...
(function () {
  skills["Spatial awareness"] = new Skill({
    ...
    description:
      "Understanding where you are in relation to other creatures and objects",
    get_effect_description: () => {
      return `Reduces environmental penalty in open areas by ^${
        Math.round(
          100 -
            (100 * get_total_skill_level("Spatial awareness")) /
              skills["Spatial awareness"].max_level
        ) / 100
      }`;
    },
  ...
  skills["Tight maneuvers"] = new Skill({
    ...
    description:
      "Learn how to fight in narrow environment, where there's not much space for dodging attacks",
    category: "Environmental",
    get_effect_description: () => {
      return `Reduces environmental penalty in narrow areas by ^${
        Math.round(
          100 -
            (100 * get_total_skill_level("Tight maneuvers")) /
              skills["Tight maneuvers"].max_level
        ) / 100
      }`;
    },
    milestones: {
  ...
  skills["Night vision"] = new Skill({
    ...
    get_effect_description: () => {
      return `Reduces darkness penalty (except for 'pure darkness') by ^${
        Math.round(
          100 -
            (100 * get_total_skill_level("Night vision")) /
              skills["Night vision"].max_level
        ) / 100
      }`;
    },
    milestones: {
  ...
  skills["Presence sensing"] = new Skill({
    ...
    get_effect_description: () => {
      return `Reduces extreme darkness penalty by ^${
        Math.round(
          100 -
            (100 * get_total_skill_level("Presence sensing")) /
              skills["Presence sensing"].max_level
        ) / 100
      }`;
    },
    milestones: {
  ...
  skills["Strength of mind"] = new Skill({
    ...
    description:
      "Resist and reject the unnatural influence. Turn your psyche into an iron fortress.",
    category: "Environmental",
    base_xp_cost: 120,
    max_level: 40,
    xp_scaling: 1.01,
    get_effect_description: () => {
      return `Reduces eldritch effects by ^${
        Math.round(
          100 -
            (100 * get_total_skill_level("Strength of mind")) /
              skills["Strength of mind"].max_level
        ) / 100
      }`;
    },
    milestones: {
...
(function () {
  ...
  skills["Swords"] = new Skill({
    ...
    get_effect_description: () => {
      return `Multiplies damage dealt with swords by ${
        Math.round(
          get_total_skill_coefficient({
            skill_id: "Swords",
            scaling_type: "multiplicative",
...
Multiplies AP with swords by ${
  ...
  skills["Axes"] = new Skill({
    ...
    get_effect_description: () => {
      return `Multiplies damage dealt with axes by ${
        Math.round(
          get_total_skill_coefficient({
            skill_id: "Axes",
            scaling_type: "multiplicative",
...
Multiplies AP with axes by ${
  ...
  skills["Spears"] = new Skill({
    ...
    category: "Weapon",
    description:
      "The ability to fight with the most deadly weapon in the history",
    get_effect_description: () => {
      return `Multiplies damage dealt with spears by ${
        Math.round(
          get_total_skill_coefficient({
            skill_id: "Spears",
            scaling_type: "multiplicative",
...
Multiplies AP with spears by ${
  ...
  skills["Hammers"] = new Skill({
    ...
    category: "Weapon",
    description:
      "Ability to fight with use of battle hammers. Why bother trying to cut someone, when you can just crack all their bones?",
    get_effect_description: () => {
      return `Multiplies damage dealt with battle hammers by ${
        Math.round(
          get_total_skill_coefficient({
            skill_id: "Hammers",
            scaling_type: "multiplicative",
...
Multiplies AP with hammers by ${
  ...
  skills["Daggers"] = new Skill({
    ...
    get_effect_description: () => {
      return `Multiplies damage dealt with daggers by ${
        Math.round(
          get_total_skill_coefficient({
            skill_id: "Daggers",
            scaling_type: "multiplicative",
...
Multiplies AP with daggers by ${
  ...
  skills["Wands"] = new Skill({
    ...
    category: "Weapon",
    description:
      "Ability to cast spells with magic wands, increases damage dealt",
    get_effect_description: () => {
      return `Multiplies damage dealt with wands by ${
        Math.round(
          get_total_skill_coefficient({
            skill_id: "Wands",
            scaling_type: "multiplicative",
  ...
  skills["Staffs"] = new Skill({
    ...
    category: "Weapon",
    description:
      "Ability to cast spells with magic staffs, increases damage dealt",
    get_effect_description: () => {
      return `Multiplies damage dealth with staffs by ${
        Math.round(
          get_total_skill_coefficient({
            skill_id: "Staffs",
            scaling_type: "multiplicative",
...
(function () {
  ...
  skills["Running"] = new Skill({
    ...
    get_effect_description: () => {
      let value = get_total_skill_coefficient({
        skill_id: "Running",
        scaling_type: "multiplicative",
      });
      return `Multiplies stamina efficiency by ${
  ...
  skills["Equilibrium"] = new Skill({
    ...
    description:
      "Nothing will throw you off your balance (at least the physical one)",
    names: { 0: "Equilibrium" },
    category: "Activity",
    max_level: 50,
    max_level_coefficient: 4,
  ...
  skills["Climbing"] = new Skill({
    skill_id: "Climbing",
    description:
      "Intense and slightly dangerous form of training that involves majority of your muscles",
    names: { 0: "Climbing" },
    max_level: 50,
    category: "Activity",
    max_level_coefficient: 2,
    ...
    get_effect_description: () => {
      ...
      return `Multiplies strength, dexterity and agility by ${
        Math.round(value * 100) / 100
      }`;
    },
  });
...
(function () {
  skills["Crafting"] = new Skill({
    ...
    get_effect_description: () => {
      return `Quality cap: ${
        get_crafting_quality_caps("Crafting").components
      }% for comps, ${get_crafting_quality_caps("Crafting").equipment}% for eq`;
    },
  });
  ...
  skills["Forging"] = new Skill({
    ...
    get_effect_description: () => {
      return `Quality cap: ${
        get_crafting_quality_caps("Forging").components
      }% for components`;
    },
    milestones: {
      10: {
          ...
          recipes: [
            {
              category: "smelting",
              subcategory: "items",
              recipe_id: "Steel ingot (inefficient)",
            },
          ],
        },
      },
...
(function () {
  skills["Iron skin"] = new Skill({
    ...
    max_level_bonus: 30,
    get_effect_description: () => {
      return `Increases base defense by ${Math.round(
        get_total_level_bonus("Iron skin")
      )}`;
    },
    milestones: {
      3: {
...
(function () {
  skills["Persistence"] = new Skill({
    ...
    max_level: 30,
    get_effect_description: () => {
      return `Increases low stamina stat multiplier to x${
        (50 +
          Math.round(get_total_level_bonus("Persistence") * 100000) / 1000) /
        100
      } (originally x0.5)`;
    },
    milestones: {
  ...
  skills["Perception"] = new Skill({
    ...
    names: { 0: "Perception" },
    description:
      "Better grasp on your senses allows you to notice small and hidden things, as well as to discern the true nature of what you obsere",
  ...
  skills["Medicine"] = new Skill({
    ...
    names: { 0: "Medicine" },
    description:
      "Create better medicaments and improve your skill at treating wounds.",
    category: "Character",
    max_level: 30,
    ...
    get_effect_description: () => {
      ...
      return `Multiplies additive effects of medicines by ${
        Math.round(value ** 2 * 100) / 100
      } and multiplicative effects by ${Math.round(value * 100) / 100}`;
    },
  });
  skills["Breathing"] = new Skill({
    ...
    names: { 0: "Breathing" },
    description:
      "Oxygen is the most important resource for improving the performance of your body. Learn how to take it in more efficiently.",
    ...
    get_effect_description: () => {
      ...
      return `Multiplies strength, agility and stamina by ${
        Math.round(value * 100) / 100
      }. Reduces thin air effects by ^${
        Math.round(
          100 -
            (100 * get_total_skill_level("Breathing")) /
              skills["Breathing"].max_level
        ) / 100
      }`;
    },
  });
...
(function () {
  skills["Haggling"] = new Skill({
    ...
    max_level: 25,
    get_effect_description: () => {
      return `Lowers trader cost multiplier to ${Math.round(
        (1 - get_total_level_bonus("Haggling")) * 100
      )}% of original value`;
    },
    max_level_bonus: 0.5,
    milestones: {
